...
rm(list = ls())
setwd(
  "G:/My Drive/VBL_users/Grad_Students/Bolivar/Dissertation/Leaf_Traits_Panama/Data/Sample_Sequencing/Post_Sequencing/Sequence_analyses"
)
...
filter_table1 <- read.csv("Filter_Table_Plate1.csv")
...
plate1[indx] <- lapply(plate1[indx], function(x)
  as.numeric(as.character(x)))

################ Plate2 ####################

filter_table2 <- read.csv("Filter_Table_Plate2.csv")
...
plate2[indx] <- lapply(plate2[indx], function(x)
  as.numeric(as.character(x)))
...
write.table(new_data_decontaminated001, "MinusNegsBlanks_001.txt", sep =
              "\t") ### This became tab "Filtered-BlankNegbyCode" in "ExtractionBatch0001_PCR102NC.xlsx".
...
write.table(new_data_decontaminated002, "MinusNegsBlanks_002.txt", sep =
              "\t")
...
write.csv(all_complete_decontaminated,
          "all_Decontaminated_byBlankandNegs_complete.csv")
trans <- t(all_complete_decontaminated)
trans <- as.data.frame(trans)
write.csv(trans, "all_toFilt_10_percent.csv") ## in the format to remove <10% OTUs -- see below
...
otu.data <- read.csv('all_toFilt_10_percent.csv',
                     row.names = 1,
                     as.is = T)
otu.data2 <- apply(otu.data, 2, function(x)
  ifelse({
    100 * x / sum(x)
  } < 0.10, 0, x))
