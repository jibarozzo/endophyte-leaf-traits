---
title: "Evaluating Endophyte-Rich Leaves and Leaf Functional Traits’ Role in Protecting Tropical Trees Against Natural Enemies"
date: today
...
library(broom.helpers) # Helpers for Model Coefficients Tibbles
...
library(permute) # Functions for Generating Restricted Permutations of Data

## Modeling:**
library(buildmer) # Stepwise Elimination and Term Reordering for Mixed-Effects Regression
library(lme4) # Linear Mixed-Effects Models using 'Eigen' and S4
library(sjPlot) # Data Visualization for Statistics in Social Science

### Specialized Statistical Packages:
library(indicspecies) # Relationship Between Species and Groups of Sites
...
# Format p-values
source(file.path(save.path,"functions/pvalue_stars.R"))
...
Illumina amplicon sequences are available in NCBI-SRA (BioProject ID: PRJNA1162076). Leaf trait data and source code for figures, analysis and reproducible manuscript are available from Zenodo: [https://doi.org/10.5281/zenodo.13905340](https://doi.org/10.5281/zenodo.13905340)
...
# ggsave(
#   filename = "./figures/Aponte_Bolivar_figure3.png",
#   plot = figure3,
#   dpi = 300,
#   units = c("mm"),
#   width = 275,
...
summa_t <-   master %>%
  ...
  tab_header(
    title = md(
      "**Table 1: Summary statistics for the relative abundance of endophytes across host tree species**"
    )
  )
...
# gtsave(table1, "tables/Aponte_Bolivar_table1.docx")
```
...
table2_ggplot <- data %>%
  ...
  tab_header(
    title = md(
      "**Table 2: Summary statistics for leaf functional traits (LFTs) across host tree species**"
    )
  )
...
gtsave(table2_ggplot, "tables/Aponte_Bolivar_table2.docx")
...
anova_table1 <- tidy(anova_m3dbRDA) |>
  ...
  select(Term, Df, "SumOfSqs", "statistic", "p.value") |>
  mutate(
    Term = case_match(
      Term,
...
table3 <- gt(anova_table1) |>
  ...
  fmt_number(columns = c("SumOfSqs", "statistic"),
             decimals = 2) |>
  fmt_number(columns = c("Df"), decimals = 0) |>
  cols_label("SumOfSqs" = "Sum. of Sqs.",
             statistic = md("*F*-statistic"),
             p.value= md("*p*-value")) |>
  tab_footnote(
    footnote = md(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(Df, SumOfSqs, statistic, p.value),
      rows = `p.value` < 0.05
    )
  ) |>
  tab_style(
  ...
  tab_header(
    title = md(
      "**Table 3: Permutational ANOVA for the best-fit dbRDA model with all host species**"
    )
  )
...
gtsave(table3, "tables/Aponte_Bolivar_table3.docx")
```
...
regrex_herb <- regrex_tables(herb_models,
                             trait_variables,
                             data_set = "Total",
                             model_type = "Herbivory")

regrex_patho <- regrex_tables(patho_models,
                              trait_variables ,
                              data_set = "Total",
                              model_type = "Pathogen")

regrex_herb_ehigh <- regrex_tables(herb_ehigh_models,
                                   trait_variables,
                                   data_set = "E-high",
                                   model_type = "Herbivory")

regrex_herb_elow <- regrex_tables(herb_elow_models,
                                  trait_variables,
                                  data_set = "E-low",
                                  model_type = "Herbivory")

regrex_patho_ehigh <- regrex_tables(patho_ehigh_models,
                                    trait_variables,
                                    data_set = "E-high",
                                    model_type = "Pathogen")

regrex_patho_elow <- regrex_tables(patho_elow_models,
                                   trait_variables,
                                   data_set = "E-low",
                                   model_type = "Pathogen")
...
table4 <- regrex_all |>
  gt(
    groupname_col = c("model_type", "data_set"),
  ...
  fmt_number(columns = everything(),
             ...
             use_seps = FALSE) |>
  cols_label(
    variables = "Leaf Functional Traits",
    estimate = "Estimate",
    rsqrd = md("R^2^~adj~"),
  ...
  tab_header(
    title = md("**Table 4: Summary of simple linear regression models for predicting leaf herbivory and pathogen damage**")
  ) |>
  tab_footnote(
...
gtsave(table4, "tables/Aponte_Bolivar_table4.docx")
```
