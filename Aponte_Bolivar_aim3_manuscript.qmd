...
table1_ggplot <- data %>%
  ...
  tab_style(
    ...
    locations = cells_body(columns = c("Observations", "Number of seedlings", "E-high seedlings", "E-low seedlings"))
  ) %>%
  tab_footnote(
  ...
  gt_plt_dist(
   ...
   tab_caption(caption = md("**Table 1: Summary statistics for leaf functional traits (LFTs) across host tree species**"))

table1_ggplot

gtsave(table1_ggplot, "tables/Aponte_Bolivar_table1.html")
...
table2 <- gt(anova_table1) |>
  ...
  tab_caption(
    caption = md(
      "**Table 2: Permutational ANOVA for the best-fit dbRDA model with all host species**"
    )
  )

table2

gtsave(table2, "tables/Aponte_Bolivar_table2.html")
```
...
# save_as_html(table3, path = "tables/Aponte_Bolivar_table3.html")
# save_as_docx(table2, path = "tables/Aponte_Bolivar_table3.docx")
```
...
table_4 <- regrex_all |>
  ...
  tab_caption(
    caption = md("**Table 4: Summary of simple linear regression models for predicting leaf herbivory and pathogen damage**")
  ) |>
  tab_footnote(
...
gtsave(table_4, "tables/Aponte_Bolivar_table4.html")
```
