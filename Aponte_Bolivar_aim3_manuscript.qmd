...
# Herbivory
map(
  c("Thickness", "Toughness", "Anthocyanins", "LMA", "Shannon"),
  ~ lm(logit_herbivory ~ ., data = master)
)
            
herb_lm_lt <- summary(lm(logit_herbivory ~ Thickness, data = masterant2))
herb_lm_lps <- summary(lm(logit_herbivory ~ Toughness, data = masterant2))
herb_lm_aci <- summary(lm(logit_herbivory ~ Anthocyanins, data = masterant2))
herb_lm_lma <- summary(lm(logit_herbivory ~ LMA, data = masterant2))
herb_lm_sdi <- summary(lm(logit_herbivory ~ Shannon, data = masterant2))

# Pathogen damage
patho_lm_lt <- summary(lm(logit_pathogenicity ~ Thickness, data = masterpat2))
patho_lm_lps <- summary(lm(logit_pathogenicity ~ Toughness, data = masterpat2))
patho_lm_aci <- summary(lm(logit_pathogenicity ~ Anthocyanins, data = masterpat2))
patho_lm_lma <- summary(lm(logit_pathogenicity ~ LMA, data = masterpat2))
patho_lm_sdi <- summary(lm(logit_pathogenicity ~ Shannon, data = masterpat2))
...
table4 <- regrex_all |>
  ...
  cols_add(dir = ifelse(
      Estimate < 0 & `p-value` >= 0.05, "white",
      ifelse(
        Estimate < 0 & `p-value` < 0.05, "#dd5129",
        ifelse(
          Estimate > 0 & `p-value` < 0.05, "#43b284",
...
table4
#gtsave(table4, "tables/Aponte_Bolivar_table4.html")
```
