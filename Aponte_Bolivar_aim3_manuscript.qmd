---
title: "Evaluating Endophyte-Rich Leaves and Leaf Functional Traits’ Role in Protecting Tropical Trees Against a Generalist Herbivore and a Pathogen"
date: today
...
## Document and Script Formatting:
library(officer)
library(broom)
library(broom.mixed)
...
### Phylogenetic & Ecological Analysis:
library(mgcv)
library(phyloseq)
library(ape)
library(metagMisc)
...
*Theobroma cacao* (Malvaceae), *Dipteryx* sp. (Fabaceae), *Lacmellea panamensis* (Apocynaceae), *Apeiba membranacea* (Malvaceae), *Heisteria concinna* (Olacaceae), *Chrysophyllum cainito* (Sapotaceae), and *Cordia alliodora* (Cordiaceae) were chosen due to their variation in LFTs (J.Wright *unpublished data*). All occur naturally at Barro Colorado Island (BCI) in central Panama (9.15 N, -79.85 W), where we collected seeds from the forest floor from multiple maternal sources in January - April 2019. Average annual precipitation at BCI is 2,600 mm and the pronounced wet season ranges from May to December [@leigh1996]. In preparation for the experiment, seeds were surface sterilized by soaking in water and rinsing in sodium hypochlorite (NaClO) and ethanol (EtOH). Seeds from each species had a species-specific sterilization protocol due to the variation in sizes and seed coats (see Appendix S1). Seed germination and the subsequent experiment were carried out at the Santa Cruz Field Facility of the Smithsonian Tropical Research Institute in Gamboa, Panama (9.118611 N, -79.703182 W). We germinated and reared seedlings in a clean and shaded greenhouse where we enclosed four tables with a PVC pipe frame and covered them with a 3 mil clear plastic sheet, for a total of two plastic enclosures with two tables each. The enclosures allowed us to grow plants at ambient temperature and natural light while providing protection from rain and most fungal spores, thus yielding zero to low FEF densities in plants that were not actively inoculated through our manipulations (see below) [@bittleston2011]. We cleaned table surfaces and walls of the enclosures on a weekly basis with 70% EtOH and 0.5 % NaClO. We germinated seedlings in sterilized trays containing a 3:1 mix of soil and river sand that was autoclaved for two one-hour cycles at 121°C prior to planting. Individual seedlings were transferred from germination trays to a 24-cell tray (each cell \~380 mL) containing the same autoclaved soil and sand mixture. We took precautions to extract complete root systems from the seedlings. See Appendix S1 for further details on plastic tray and pot sterilizations protocols.
...
For the pathogen assays, we inoculated seedlings with *Calonectria* sp. pathogen. *Calonectria* is a widespread pathogen in tropical and temperate ecosystems [@crous2006; @li2022]. Its distribution reflects trends in agriculture, forestry, and the ornamental plant trade [@li2022]. In the tropics its effects are known as *Calonectria* leaf blight (CLB) which has become a primary fungal disease in commercial tree plantations [@sanchez-gonzalez2022; @lombard2010]. We prepared the pathogen inoculum following Gilbert & Webb [-@gilbert2007] and Tellez et al. [-@tellezRedCol2016]. Briefly, this required growing inoculum in 2% MEA in caps (agar plug) from 1.8 mL cryovials, then grown for 5 days before applying to seedlings. In our assays, haphazardly selected 2 similarly aged/sized leaves from seedlings not used in herbivore assays within 10-14 days after ambient spore-fall inoculations (see Fungal endophyte inoculation section above) [@gilbert2007]. Then we lightly punctured the leaf lamina with a sterile needle (95% EtOH and heat treated) and applied an agar plug inoculated with hyphae of *Calonectria* sp. (*P+* treatment), and in a separate leaf, an agar plug without the pathogen (*P-* control). Agar plugs were held in place with sterile metal double prong curl clips (Sally Beauty LLC., Denton, Texas, USA. Item number: SBS-292507). Leaves with the *P+* or *P-* treatments were misted with sterile water two times a day (morning and afternoon) to maintain moisture. After four days, we removed the plugs and took digital photos to analyze leaf area damage using ImageJ [v1.52r; @schneider2012].
...
rep_statement <- hux(
  "Scale of inference" = c(
    "Leaf functional traits (LFTs)",
    "Genomic data",
    "Herbivore assays",
    "Pathogen assays"
  ),
  "Scale at which the factor of interest is applied" = c(
    "Host Species",
    "Species (OTUs)",
    "Species/individual",
    "Species/individual"
  ...
  "Number of replicates at the appropriate scale" = c(
    "Treatment = 2 (E-low, E-high), Tropical tree species = 7, Replicates per species = 5. Replicate leaves per plant = 3",
...
Thirdly, we arbitrarily designated percent leaf damage in herbivore assays as high (\>70%), medium (31-69%) and low (\<30%) and in pathogen assays as high (\>30%) and low (\<30%). These categories allowed us to explore correlations between host tree species and specific FEF OTUs. To achieve this, we used the `multipatt` function from the `indicspecies` package in *R* [v. 1.7.14; @decaceres2009], which allowed us to determine correlation indices for each site group combination (host tree species and OTUs). We applied the `multipatt` function with arguments `func`= "r.g" and `control` = how(nperm=999) to our OTU abundance matrix to calculate the point biserial correlation coefficient for each OTU at all tree species and treatment group combinations [@decaceres2009]. Like with other statistical tests performed, we adjusted *p* values to account for false discovery rates in multiple comparisons by using "BH" method [@benjamini1995] in the `p.adjust` function from the `stats` package [@rcoreteam2024]. We then filtered the adjusted *p* value with a cutoff of \< .05.
...
Lastly, to test how LFTs and FEF communities interact to influence herbivory and pathogen damage in tropical tree species, we used a generalized linear mixed model (GLMM) with herbivory and pathogen damage percentage (logit transformed) as the response variable. To determine which fixed effects to include in the GLMMs we used the `vif` function from the `car` package in *R* to calculate the variance inflation factor for all explanatory variables (ACI, LT, LPS, LMA, and Shannon diversity index) [@fox2019; @rcoreteam2024]. Complementary to this, we calculated Pearson's coefficient for each pair of LFTs with by creating a correlation matrix and applying the `cor` function from the `stats` package to assess correlations among traits [@rcoreteam2024]. We opted to maintain explanatory variables LT and LMA, and exclude ACI and LPS from subsequent general linear models (GLMMs) due to high collinearity between the two variables with LMA, *r*(1112) = 0.68, *p* \< .0001 and *r*(1113) = 0.65, *p* \< .0001, respectively. We modeled only main effects with explanatory variables, we did not model interactions effects to avoid overfitting models. We used a restricted maximum likelihood estimates for model fit with the `lme` function from the `nlme` package [@pinheiro2023; @pinheiro2000]. For our logit herbivory GLMMs we used tree species as a random effect and modeled tree species variance structure with the `varIdent` argument [@pinheiro2023; @pinheiro2000]. For our logit pathogen damage GLMMs we use tree species as a random effect and modeled a nested variance structure for pathogen treatment within treatment groups per species with the `varIdent` argument [@pinheiro2023; @pinheiro2000]. We corrected for phylogenetic non-independence by incorporating phylogenetic correlation structure in all our models. We manually compared and selected models based on Akaike Information Criterion (AIC) with a penalty of 2 degrees of freedom (ΔAIC) with the `model.sel` function from the `MuMIn` package [@barton2023]. We selected the best-fit model based on the lowest value obtained. All visualizations and tables were performed in R [v. 4.4.1; @rcoreteam2024] using the `ggplot2`, `gt`, `gtExtras`, `huxtable` and `flextable` packages [@mock2023; @hugh-jones2023; @iannone2023; @gohel2024; @wickham2022] 

# Results

Inoculation of seedlings was successful. Seedlings exposed to forest spore fall (i.e., *E-high*) had a significantly higher proportion of leaf segments colonized by FEF across all species (data from cultures, Figure S1). Similarly, the  molecular data set showed that seedlings with *E-high* treatment had a significantly higher FEF relative abundance (paired, two-sided *t*-tests, *p* \< .05) for all tree species when compared to the *E-low* treatment (Figure 1a and @tbl-table1). Despite these significant differences, there was a high degree of variability in FEF relative abundance within each treatment type (Figure 1 and @tbl-table1).

We observed general differences in LFTs among species (@tbl-table2). Anthocyanins (ACI) and leaf punch strength (LPS) did not differ between treatments (*E-low* and *E-high*) when we combined all host species (Figure S2a and Figure S4a respectively). For LT and LMA we saw significant differences between *E-low* and *E-high* treatment groups when we combined all host species (Fig S3a and Figure S5a, respectively). As predicted, we did observe lower herbivory in the *E-high* treatment compared to the *E-low* treatment when we combined all host species, *t*(31) = 2.23, *p* = .033 (Figure 2a). We did not observe differences in pathogen damage between *E-low* and *E-high* treatments when we combined all host species (Figure 2b). However, within the *E-low* treatment group, leaves exposed to *Calonectria* sp. suffered greater damage (*M* = 17.37, *SD* = 12.57) compared to non-exposed leaves (*M* = 3.49, *SD* = 5.42), *t*(31) = -7.19, *p* \< .0001 (Figure 2b). The same pattern arose for the *E-high* treatment group: exposed leaves (*M* = 20.04, *SD* = 15.72), non-exposed leaves (*M* = 7.17, *SD* = 17.48), *t*(31) = -3.26, *p* \< .01 (Figure 2b). At the species level, herbivory and pathogen damage did not differ between treatment groups (Figure S6a-S6b).

All LFTs are significantly correlated with FEF community composition (Figure 3 and @tble-dbRDA). We used dbRDA to understand FEF community composition across host species and treatment groups. The analyses revealed that 6.9% of the overall model variance in FEF communities was constrained by the LFTs. The first axis (dbRDA1) explained 45.2% and the second axis (dbRDA2) explained 19.6% of the constrained variance (Figure 3). We observed a high degree of overlap between FEF communities (Figure 3), indicating that the communities are similar in composition across host species and treatment groups, at least at the degree of resolution provided by ITS data (Figure 3). Nonetheless, we also observed tight clustering of FEF communities in *C. cainito* and *L. panamensis*, emphasizing a distinct composition of FEF OTUs within the FEF identified. Other host species showed greater variation in FEF composition. Our PERMDISP analyses showed no significant differences in host species group dispersion (*F*~6,~ ~149~ = 0.717, *p* \< .63), observed differences were due to location. Unsurprisingly, the dispersion for *E-low* and *E-high* treatment groups was significantly different (*F*~1,~ ~154~ = 5.09, *p* \= .03). The Tukey tests revealed dispersion of host species were not significantly different at the α \< .05 level, while treatment group dispersions were.
...
Host species leaf traits encompassed variation along the leaf economic spectrum (LES). The PCA revealed how ACI, LT, LPS and LMA were related. We plotted leaf trait data according to tree species groups on the PCA axes to show how the variance in the complete data set is explained by PC1 (60%) and PC2 (27%) (Figure 4a). We observed that ACI, LPS and LMA loadings tracked along PC1 towards more negative values, showing correlation among these traits (Figure 4a). Traits LT and LPS were orthogonal to each other (Figure 4a), indicative of low correlation. We note distinct grouping of host species along PC1 towards negative values. We observed compact clustering of host species on opposite ends of PC2 (Figure 4a). We note similar relationships between the leaf traits with respect to PC1 and PC2 in the subset of individual seedlings used for herbivory versus pathogen damage trials (Figure 4b-c). The PCA of leaf traits from seedlings used in herbivory trials had a PC1 explaining 57.5% of the variance and a PC2 explaining 28% of the variance in the subset data (Figure 4b). We saw an inversion of the LT loading in direction of positive values, as well as the same tree species clustered (i.e., *Dipteryx* sp. and *A. membranacea*) along PC2 (Figure 4b) with respect to Figure 4a. The PCA of leaf traits from seedling used in pathogen damage trials had a PC1 explaining 64% of the variance and a PC2 explaining 25% of the variance in the subset data (Figure 4c). We detected similar relationships among leaf traits and PC axes in the pathogen damage subset data (Figure 4c) when compared to the complete data set (Figure 4a).


Leaf functional traits influence plants' response to herbivory and pathogen damage differentially. We examined the relationship between LFTs, herbivory and pathogen damage to understand how host species on opposite ends of the LES modulate herbivory and pathogen damage. We used simple linear regressions plotting herbivory and pathogen damage with PC1 and PC2 from the PCA. Simple linear regressions of herbivory (%) against PC1 revealed no correlation (Figure 5a; *R^2^~adj~* = -0.0024, *F*~1,~ ~208~ = 0.508, *df* = 208, *p* = .447), where positive values represent greater values of ACI, LPS and LMA. Even though we note large spread in the data (Figure 5a and 5b), we saw that herbivory was strongly associated with PC2 (Figure 5b; *R^2^~adj~* = 0.079, *F*~1,~ ~208~ = 18.9, *p* \< .0001), where positive values represent greater LT. Regressions of pathogen damage (%) plotted against PC1 revealed a significant correlation (Figure 5c; *R^2^~adj~* = 0.064, *F*~1,~ ~380~ = 26.93 *p* \< .0001), in which positive values represent greater values of ACI, LPS and LMA. We did not see a significant relationship between pathogen damage and PC2 (Figure 5d; *R^2^~adj~* = 0.002, *F*~1,~ ~380~ = 1.60, *p* = .207). 

We observed similar patterns when we performed simple linear regressions on the raw LFTs and logit transformed herbivory and pathogen damage data (@tbl-linearreg; Figure S7 - S8, respectively). We observed a significant positive relationship between herbivory and LT (@tbl-linearreg and Figure S7a; *p* \< .0001) when considering the complete data set. We did not observe a significant relationship between herbivory and LPS, ACI, LMA, and Shannon diversity index for FEF (@tbl-linearreg and Figure S7b - S7e) for the complete data set. However, we did observe a general decline in herbivory as FEF diversity increased which aligns with our first prediction. Furthermore, we saw a significant negative relationship between herbivory and Shannon diversity for the *E-low* treatment group (@tbl-linearreg and Figure S7e; *p* \< .0001). We also saw an increase in herbivory for the *E-high* treatment group as Shannon diversity index for FEF increased, but this was not statistically significant (@tbl-linearreg and Figure S7e; *p* = 0.062), a result contrary to our expectations.

We noted significant negative relationships between pathogen damage and LPS (@tbl-linearreg and Figure S8b; *p* \< .0001), ACI (@tbl-linearreg and Figure S8c; *p* = .0002) and LMA (@tbl-linearreg and Figure S8d; *p* \< .001) when considering the complete data set. Pathogen damage did not have a significant correlation with LT (@tbl-linearreg and Figure S8a; *p* = .482). The *E-low* and *E-high* treatment groups follow the same general trend as the complete data set (@tbl-linearreg and Figure S8). Contrary to our predictions, we found a statistically significant positive relationship between pathogen damage and Shannon diversity index in the complete data set (@tbl-linearreg and Figure S8e; *p* \< .01). Upon further scrutiny, only the *E-high* treatment group had a significant positive correlation between pathogen damage and Shannon diversity index (@tbl-linearreg and Figure S8e; *p* \< .01), while the *E-low* treatment did not have a significant correlation (@tbl-linearreg and Figure S8e; *p* = 0.81) . 

Leaf thickness and LMA have opposite outcomes in plants' response to herbivory. The best-fit for our GLMMs showed that fixed effects LT, LMA, and *E-high* treatment group were statistically significant predictors of herbivory damage (@tbl-tableGLMM). No measure of FEF abundance or diversity was present in the final model. Leaf mass per area was a significant positive predictor of herbivory with the greatest effect size (estimate = 823.6, *t*(200) = 2.65 *p* \< .01). While LT is a significant negative predictor of herbivory damage (estimate = -0.01, *t*(200) = -3.16, *p* \< .01) and *E-high* as well (estimate = -0.75, *t*(200) = -6.16, *p* \< .001). The best fit model for pathogen damage did not reveal any of the LFTs as significant predictors (@tbl-tableGLMM). Like our previous model, no measure of FEF abundance or diversity was present in final model. Even though it was not significant, LMA showed the greatest effect size (estimate = 225.62, *t*(352) = 1.61, *p* = .25).
...
This is the first study to examine the interplay of FEF communities and LFTs in response to the effects of a generalist herbivore and a pathogen in tropical tree species. Integrating the role of FEF communities into a conceptual framework that includes trade-offs to plants' constitutive and induced defenses in response to natural enemies helps us understand the importance of FEF communities in the maintenance of plant diversity in tropical forests. Our findings suggest that foliar endophytic fungi (FEF) communities work to improve leaf defenses against generalist herbivores and pathogens in tropical trees. Across all host species, we saw that the *E-high* treatment group exhibited lower herbivory damage than the *E-low* treatment group. This is in partial support to our prediction that leaf-cutter ants cause less leaf damage in plants with higher FEF abundance; it is less clear how higher level of FEF richness and diversity influence leaf herbivory. At the host species level, we saw that *E-high* had less herbivory damage, however these differences were not significant between treatment groups. This is probably due to a small replicate size in treated individuals per species used in herbivory assay trials. Our results align with Estrada et al., [-@estrada2013], where leaf-cutter ant effects were significantly reduced in paper disks and leaves treated with higher densities of a common endophyte. In a laboratory setting, Bittleston et al., [-@bittleston2011] found similar patterns in herbivory reduction using *C. alliodora* treated with high and low FEF loads and laboratory reared colonies of *Atta colombica*. In a field study, Coblentz and Van Bael [-@coblentz2013] found that leaf-cutter ants preferred leaves with a lower FEF abundance when compared to surrounding leaf material (material not selected by ants). Rocha et al. [-@rocha2017], found endophytic *Trichoderma* more frequently isolated from leaf cuttings rejected by queen-less leaf-cutter ant colonies of *A. sexdens rubropilosa* in southeast Brazil. Our results track findings from previous studies that focus on single host-endophyte interactions or haphazard field collection and build upon them by using multiple host tree species and field inoculated FEF communities. 

The leaf economic spectrum (LES) provides a framework for interpreting results based on the functional traits that can have an important role in understanding tripartite plant-insect-pathogen interactions. The LES describes how LFTs with high values are characteristic of long-lived versus short-lived leaves [@wright2004, @kitajima2013, @poorter2006]. The LES, thus acts as a host-imposed filter which FEF communities must overcome to colonize leaf tissue, assemble, and carry out their life cycle. Through this lens we found partial support for predictions: 2) *tree species with leaf functional traits on the low end of the LES would have less herbivory and pathogen damage when treated with high FEF loads (*E-high*) compared to their low FEF counterparts (*E-low*)*; and 3) *tree species with leaf functional traits on the high side of the LES treated with high FEF loads (*E-high*) would have no differences in herbivory and pathogen damage compared to their low FEF counterparts (*E-low*)*. While we did not measure leaf lifespan directly, we interpret the values obtained for the LFTs we measured as proxies for leaf lifespan, with the caveat that the traits were measured on seedlings and leaves were relatively young (\< 150 days old). We saw that species with relatively low values of ACI, LMA, and LPS (Figures S2 - S5) constrained in the PC1 axis (Figure 4b) experienced very little herbivory damage (i.e., *Dipteryx* sp.) or had much variability (i.e., *C. alliodora*) (Figure 5a), regardless of endophyte treatment. Overall, Species with relatively high values of ACI, LMA, and LPS like *C. cainito* experienced low to moderate herbivory damage (Figure 5a). 
<!-- We view these findings and those presented in Figure S7 as support for prediction 4) *leaves on the high end of the LES, are less attractive to leaf cutter-ants but the effects of FEF communities outweigh this selection factor* after a certain threshold is reached, as mentioned above. A similar pattern was evident in the pathogen damage data set (Figure 5c). The contrast in the slopes of the regression lines in Figure 5a and 5c highlight the potential importance that LES traits have to counter herbivory and pathogen damage.  -->

Our GLMM analysis allowed us to look at FEF and leaf traits at the same time; we observed only LMA, LT and *E-high* treatment group as strong predictors for herbivory damage (@tbl-tableGLMM). Using data from all the assays in simple linear regressions, FEF diversity and community composition correlated negatively with herbivory damage, since we saw reduced herbivory as Shannon diversity index increased (@tbl-linearreg; black regression line in Figure S7e). As a caveat to these results, we saw an inverse relationship between the treatment groups with the *E-high* group positively correlating to herbivory and *E-low* group negatively correlating to herbivory (@tbl-linearreg; yellow and pink regression lines, respectively, in Figure S7e). The contrast between GLMM and simple linear regressions point to the nuanced role of FEF diversity in combating herbivory; LFTs, LMA and LT, overshadowed FEF diversity in our results. The FEF abundance, measured as *E-high* treatment group, also played a strong role in predicting herbivory, but less so diversity and community composition. <!-- Given consistent preference by leaf-cutter ants [@bittleston2011; @coblentz2013; @estrada2013], we interpret our results as evidence for a threshold of diminishing returns for the host plant, were the acquisition of more FEF increases the chances of hosting a community that does not deter leaf-cutter ants. From the perspective of the leaf-cutter ants, everything is equally infected and the cost of allocating new resources is taxing. It highlights a trade-off in the presumably asymptomatic symbiosis host species have with FEF communities. -->

We did not observe the same pattern for pathogen damage since there were no significant differences between treatment groups when considering all tree species combined (Figure 2b). Like herbivory damage, pathogen damage was not significantly different across treatment groups per species. Interestingly we saw that the *E-high* group had equal or slightly higher pathogen damage than the *E-low* group in all tree species (Figure S6b). The best-fit GLMM for pathogen damage showed LMA and LT as weak predictors (@tbl-tableGLMM). On the other hand, with simple linear regressions we saw a significant increase in pathogen damage as Shannon diversity index increased for the complete data set (@tbl-linearreg and Figure S8e). Like our herbivory damage models, the contrast between GLMM and simple linear regression point to a nuanced role of FEF abundance and diversity in combating a pathogen. Pathogen damage significantly increased in the *E-high* group as Shannon diversity increased, but the *E-low* group did not (@tbl-linearreg and Figure S8e). González-Tauber [-@gonzalez-teuber2016] found the opposite trend when examining *in vitro* the inhibitory effects of the most abundant genera (*Mycosphaerella* sp., *Xylaria* sp., *Diaporthe* sp., and *Penicillium* sp.) in FEF communities isolated from the southern temperate tree *Embothrium coccineum* against three common pathogens (*Botrytis cinerea*, *Fusarium oxysporum* and *Ceratocystis pilifera*). In our case, it is possible that the FEF abundance and diversity acquired from spore-fall have a synergistic interaction with *Calonectria* sp., thus outweighing any benefits provided by single-species interactions, ultimately increasing pathogen damage intensity. Arnold et al. [-@arnold2003] found significant differences in pathogen damage between *E-high* and *E-low* treatments groups of *T. cacao* when exposed to *Phytophthora* sp. Our study focused on the effect of a pathogen on relatively young leaves *versus* Arnold et al. [-@arnold2003] where they observed leaves from different life stages and the effects of the pathogen increased relative to leaf age. A lack of leaves at different life stages as well as a low sample size might explain the lack of differences in pathogen damage present in this study.  

Comprehending the relationship between the LES, plant defenses, and FEF communities is crucial for understanding the complex interactions among plants, insects, and pathogens. The Optimal Defense Theory (ODT), as outlined by Stamp [-@stamp2003], proposes three key predictions about plant defenses. First, a plant's defense investment is directly proportional to the frequency of attacks, such as herbivory or pathogen intensity, and inversely related to the cost of resources [@holeski2010]. Second, plants tend to allocate resources preferentially to parts with high reproductive value, especially when defense costs are minimal. Third, plants exhibit increased defensive responses after being attacked. This framework suggests that the likelihood of a plant to bolster its defenses following an attack is inversely related to its inherent defense traits [@holeski2010]. Our results point to a preemptive low-cost investment strategy against plant enemies, particularly at the seedling stage, that is leveraged by species specific inherent defense mechanisms. We did not track herbivory or pathogen damage past the seedling stage, so an avenue for inquiry is to investigate how FEF communities change after herbivore and pathogen attacks and to determine the key life stages. A combination of *in vitro* and *in vivo* assays could help elucidate the roles of specific FEF OTUs. The use of *in vitro* assays could help identify the potential anti-herbivore and anti-pathogen qualities of specific FEF OTUs. *In vivo* inoculation assays could help identify the importance that specific FEF OTUs have in different developmental stages, especially in older stages.

# Conclusion

This study advances our understanding of the intricate interactions between plants and their FEF communities, particularly in the context of plant defense mechanisms against herbivores and pathogens. Previous studies focused on single host species or limited number of endophytes, our study uses multiple host species and associated FEF community to have a better picture of community level dynamics. Our findings highlight the complex dynamics of plant-herbivore-pathogen relationships and underscore the importance of FEF communities as a potentially low-cost, preemptive defense strategy for plants, especially during early growth stages. These insights not only shed light on the nuanced role of endophytes in plant ecology but also open avenues for future research, particularly in exploring the strategic resource allocation in plants and the specific contributions of FEF to plant resilience. As we continue to unravel these complex biological interactions, the knowledge gained holds promise not only for ecological theory but also for practical applications in agriculture, forestry, and conservation of the tropics.
...
Illumina metagenome sequences are available in NCBI-SRA (BioProject ID: PRJNA1162076). Leaf trait data and source code for figures, analysis and reproducible manuscript are available from Zenodo:
...
# Format p-values
source(file.path(save.path,"functions/fmt_stars.R"))

# Loading files
csamp <-
  readRDS(file.path(save.path, "clean_data/clean_sequences.rds"))

tbas <- readRDS(file.path(save.path, "clean_data/tbas_filtered.rds"))

data <-
  readRDS(file.path(save.path, "clean_data/leaf_data_raw_analyses.rds"))

ndata <-
  readRDS(file.path(save.path, "clean_data/new_leaf_data.rds"))

ndata2 <-
  readRDS(file.path(save.path, "clean_data/new_leaf_data2.rds"))

ants <- readRDS(file.path(save.path, "clean_data/ants_data.rds"))
...
patho <-
  readRDS(file.path(save.path, "clean_data/pathogen_data.rds"))

# Just the area damaged
patho2 <- patho |>
  select(c(1, 3, 4, 8, 9, 12))

pq1 <- readRDS(file.path(save.path, "clean_data/phyloseq_main.rds"))


pq2 <- readRDS(file.path(save.path, "clean_data/phyloseq_pq2.rds"))
...
eOTU2 <- readRDS(file.path(save.path, "clean_data/eOTU2.rds"))

cOTU <- readRDS(file.path(save.path, "clean_data/clean_OTU.rds"))

rabun <- readRDS(file.path(save.path, "clean_data/rabun_tidy.rds"))

rawra <- readRDS(file.path(save.path, "clean_data/rawra_tidy.rds"))
...
master <- df_list |>
  ...
  rename(
    Pathogen_Damage_area = Damage_area,
    E_load = E_load.x,
...
# Saving
ggsave(filename = "./figures/Aponte_Bolivar_figure1.png", plot = fig1, dpi = 300, units = c("mm"), width = 210, height = 180, bg = "white")
#
```
  ...
  ggplot(
    master |>
      ...
      select(
        ...
        E_load,
        Treatment,
        Pathogen_percent_damage
      ) |>
      pivot_longer(
        !c(Species, Sample_name, E_load, Pathogen_percent_damage),
        names_to = c("Vars"),
        #names_sep = "_",
        values_to = "Values"
      ) |>
      pivot_wider(names_from = Values, values_from = Pathogen_percent_damage) |>
      mutate(denoise = abs(pathogen - control)), # Absolute difference in pathogen damage between control and pathogen treated leaves
    aes(x = E_load, y = denoise, fill = E_load)
  ) +
  geom_point(
    aes(color = E_load),
    alpha = 0.7,
...
# Figure 2a-2b
figure2 <- ggarrange(herb_boxplot, patho_boxplot,
  nrow = 1,
  ncol = 2,
...
figure2
# ggsave(filename = "./figures/Aponte_Bolivar_figure2.png", plot = figure2, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
...
lfts <- ndata2 |>
  select(Thickness, Toughness, Anthocyanins, LMA, E_load) |>
  na.omit()
...
#| fig.width: 6.5
#| fig.height: 6
#| fig.cap: "FEF community composition was associated with leaf functional traits (LFTs) from the leaf economic spectrum (LES) (genomic data set). Variation in FEF community composition within and between host species (*n* = 7) and treatment groups (*E-low*, *E-high*) from distance-based redundancy analyses (dbRDA). Solid black arrows represent statistically significant associations (*p* <.01). The arrows are displayed at double their actual size to enhance visual clarity. Each point represents a FEF community sample from one host tree species per treatment group; colors represent host tree species. Circles and filled triangles represent low (*E-low*) and high (*E-high*) FEF treatment groups, respectively."
...
m3dbRDA <- dbrda(
  otu_e.dist ~ Anthocyanins + Toughness + Thickness + LMA + E_load,
  data = ndata2,
  dist = "bray",
  na.action = na.omit
)
...
# Environmental vectors or factors onto an ordination
# Tell us the vector correlations and Rsquared
env_fit_test <- envfit(m3dbRDA, lfts, na.rm = TRUE, perm = 999)
...
A4.sub1 <- A.4[4, ] # Thickness
A4.sub2 <- A.4[1, ] # Anthocyanins
A4.sub3 <- A.4[3, ] # LMA
A4.sub4 <- A.4[5, ] # Toughness
...
ggsave(
  filename = "./figures/Aponte_Bolivar_figure3.svg",
  plot = figure3,
  dpi = 300,
  units = c("mm"),
  width = 275,
...
# ggsave(
#   filename = "./figures/Aponte_Bolivar_figure3_points.png",
#   plot = p_m3dbrda,
#   dpi = 300,
#   units = c("mm"),
#   width = 250,
...
auto <- autoplot((pca),
  data = data.frame(ndata), # For some reason this functions require data to be data= data.frame()
  ...
  scale_shape_manual(name = "Treatment", labels = list("E-low", "E-high"), values = c(16, 17)) +
...
# Plot
auto_ant <- autoplot((ant.pca),
  data = data.frame(masterant2), # For some reason this functions require data to be data= data.frame()
  alpha = 0, # Setting alpha to zero render the automatic circle point null.Manipulate shapes with geom_point().
  loadings = TRUE, loadings.colour = "black",
  loadings.label = TRUE, loadings.label.colour = "black",
...
# Plot
auto_pat <- autoplot((pat.pca),
  data = data.frame(masterpat2), # For some reason this functions require data to be data= data.frame()
  alpha = 0, # Setting alpha to zero render the automatic circle point null.Manipulate shapes with geom_point().
  loadings = TRUE, loadings.colour = "black",
  loadings.label = TRUE, loadings.label.colour = "black",
  loadings.label.size = 4, size = 7, loadings.label.vjust = 0,
...
# ggsave(filename = "./figures/Aponte_Bolivar_figure4.png", plot = figure4, dpi = 300, units = c("mm"), width = 275, height = 300, bg = "white")
...
ggsave(filename = "./figures/Aponte_Bolivar_figure5.png", plot = figure5, dpi = 300, units = c("mm"), width = 300, height = 225, bg = "white")
...
#| warning: false
#| label: tbl-table1
#| tbl-cap: "Summary statistics for the relative abundance of endophytes across host tree species and inoculation treatment**"
...
summa_t <-   master %>%
  ...
  mutate(
    ...
    Treatment = case_match(Treatment, "E-" ~ "E-low", "E+" ~ "E-high")
  ) %>%
  dplyr::group_by(Species, Treatment) %>%
  summarize(
    "Mean (±)" = paste0(round(mean(
      na.exclude(Relative_Abundance)
    ), 3), " (±", round(sd(
      na.exclude(Relative_Abundance)
    ), 3), ")"),
    "Median (±)" = paste0(round(median(
      na.exclude(Relative_Abundance)
    ), 3), " (±", round(sd(
      na.exclude(Relative_Abundance)
    ), 3), ")"),
  ...
  tab_style(
    ...
    locations = list(
      cells_body(columns = c("Mean (±)", "Median (±)")),
  ...
  tab_style(
    style = list(cell_text(weight = "bold"), cell_text(style = "italic")),
  ...
  tab_header(
    title = md(
      "**Summary statistics for the relative abundance of endophytes across host tree species**"
    )
  )
...
gtsave(table1, "tables/Aponte_Bolivar_table1.html")
```
...
#     ) |>
#     rename(
#       "Anthocyanins (ACI)" = Anthocyanins,
#       "Leaf Thickness (LT) (\U00B5m)" = Thickness,
#       "Leaf Punch Strength (LPS) (N mm^-1)" = Toughness,
#       "Leaf Mass per Area (LMA) (mg/mm)" = LMA
#     ) |>
#     mutate(
#       Treatment = case_match(Treatment, "E-" ~ "E-low (n = 570)", "E+" ~ "E-high (n = 544)")
#     ) |> # The sum of E- and E+ was added after iterations of tbl_summary().
#     na.omit(),
#   by = Species,
#   missing = "no"
# ) |>
...
# trying to visualize table differently
# Function for removing outliers for purposes of this table (https://stackoverflow.com/questions/4787332/how-to-remove-outliers-from-a-dataset)
...
table2_ggplot <- data %>%
  ...
  select(Species,
         ...
         LMA) %>%
  mutate(
    Anthocyanins = remove_outliers(Anthocyanins),
    Thickness = remove_outliers(Thickness),
    Toughness = remove_outliers(Toughness),
    LMA = remove_outliers(LMA)
  ) %>%
  mutate(
    Species = case_match(
      Species,
  ...
  summarise(
    Observations = n(),
    "Number of seedlings" = (n() / 6) / 3,
    # 6 measurements (mainly DYPTE) per leaf per seedling, 3 leaves per seedlings per species
    "E-high seedlings" = sum(E_load == "E+") / 6 / 3,
    "E-low seedlings" = sum(E_load == "E-")  / 6 / 3,
    "Mean (±)" = paste0(round(mean(na.exclude(
      Values
    )), 4), " (±", round(sd(na.exclude(
      Values
    )), 4), ")"),
    "Median (±)" = paste0(round(median(na.exclude(
      Values
    )), 4), " (±", round(sd(na.exclude(
      Values
    )), 4), ")"),
    "LFT distribution" = list(Values)
  ) %>%
  gt() %>%
  ...
  tab_footnote(
    footnote = "LFT = leaf functional traits. Outliers have been removed to better visualize distribution of LFT values.",
    locations = cells_column_labels(columns = "LFT distribution"),
    placement = "auto"
  ) %>%
  ...
  gt_plt_dist(
    ...
    fig_dim = c(5, 30),
    same_limit = FALSE,
    # ensures each histogram can have its own axis range
    fill_color = "#dd5129",
    # changed color for a little more visibility of the bins
    line_color = "black"
  ) %>%
  tab_header(
    title = md(
      "**Summary statistics for leaf functional traits (LFTs) across host tree species**"
    )
  )

table2_ggplot

gtsave(table2_ggplot, "tables/Aponte_Bolivar_table2.html")
...
#| warning: false
#| label: tbl-dbRDA
#| tbl-cap: "Permutational ANOVA for the best-fit dbRDA model with all host species."
#dbRDA table  (results from terms are the same as adonis2 but without R2)

anova_m3dbRDA <- readRDS(file.path(save.path, "/clean_data/anova_m3dbRDA.rds"))

anova_table1 <- tidy(anova_m3dbRDA) |>
  as.data.frame() |>
  # mutate("p-value" = str_replace_all(p.value, "0.0001", "****"), across(term, ~ gsub("_m", " (m)", .))) |>
  rename(
    Term = term,
    "Sum. of Sqs." = SumOfSqs,
  ...
  select(Term, Df, "Sum. of Sqs.", "F-statistic", "p-value") |>
  mutate(
    Term = case_match(
      Term,
      "Anthocyanins" ~ "Anthocyanins (ACI)",
      "Thickness" ~ "Leaf Thickness (LT) (\U00B5m)",
      "Toughness" ~ "Leaf Punch Strength (LPS)(N mm^-1)",
      "LMA" ~ "Leaf Mass per Area (LMA)(mg/mm)",
      "E_load" ~ "Endophyte load (High/Low)",
      "Residual" ~ "Residuals"
    )
  ) 
table3 <- gt(anova_table1) |>
  fmt_stars(columns = c("p-value")) |>
  fmt_number(columns = c("Sum. of Sqs.", "F-statistic"),
             decimals = 2) |>
  fmt_number(columns = c("Df"), decimals = 0) |>
  cols_label(`F-statistic` = md("*F*-statistic"),
             pval = md("*p*-value")) |>
  tab_footnote(
    footnote = md(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(`Df`, `Sum. of Sqs.`, `F-statistic`, `p-value`),
      rows = `p-value` < 0.05
    )
  ) |>
  tab_style(
    style = cell_text(style = "italic"),
    locations = cells_body(column = `Term`, rows = `Term` == "Residuals")
  ) |>
  sub_missing(columns = 4, missing_text = "-") |>
  tab_header(
    title = md(
      "**Permutational ANOVA for the best-fit dbRDA model with all host species**"
    )
  )

table3

gtsave(table3, "tables/Aponte_Bolivar_table3.html")
```
...
#| warning: false
#| label: tbl-linearreg
#| tbl-cap: "Summary of simple linear regression models for predicting leaf herbivory and pathogen damage. Positive correlation denoted by upward light green arrow, negative correlation by downward light orange arrow. Models are separated by treatment and data set evaluated. Significance levels are represented by *ns* (not significant) and asterisks [*p* < .05 (\\*), *p* < .01 (\\*\\*), *p* < .001 (\\*\\*\\*), and *p* < .0001 (\\*\\*\\*\\*)]."

# Linear regression models
# Summary function
model_summary <- function(data, response, predictor){
  lm_model <- lm(data[[response]] ~ data[[predictor]], data = data)
  summary(lm_model)
}


# Table function
regrex_tables <- function(model,
                          data_set,
                          type_damage5
                          ) {
  data.table(
    LFTs = c(
      "Anthocyanins (ACI)",
      "Leaf Thickness (LT) (\U00B5m)",
      "Leaf Punch Strength (LPS)(N mm^-1)",
      "Leaf Mass per Area (LMA)(mg/mm)",
      "Shannon Diversity Index"
    ),
    data_set = data_set,
    type_damage = type_damage5,
    estimate = c(
      model[[1]]$coefficients[2],
      model[[2]]$coefficients[2],
      model[[3]]$coefficients[2],
      model[[4]]$coefficients[2],
      model[[5]]$coefficients[2]
      
    ),
    rsqrd = c(
      model[[1]]$adj.r.squared,
      model[[2]]$adj.r.squared,
      model[[3]]$adj.r.squared,
      model[[4]]$adj.r.squared,
      model[[5]]$adj.r.squared
      
    ),
    
    fstat = c(
      model[[1]]$fstatistic[1],
      model[[2]]$fstatistic[1],
      model[[3]]$fstatistic[1],
      model[[4]]$fstatistic[1],
      model[[5]]$fstatistic[1]
    ...
    pval = c(
      pf(
        model[[1]]$fstatistic[1],
        model[[1]]$fstatistic[2],
        model[[1]]$fstatistic[3],
        lower.tail = FALSE
        
      ),
      pf(
        model[[2]]$fstatistic[1],
        model[[2]]$fstatistic[2],
        model[[2]]$fstatistic[3],
        lower.tail = FALSE
        
      ),
      pf(
        model[[3]]$fstatistic[1],
        model[[3]]$fstatistic[2],
        model[[3]]$fstatistic[3],
        lower.tail = FALSE
        
      ),
      pf(
        model[[4]]$fstatistic[1],
        model[[4]]$fstatistic[2],
        model[[4]]$fstatistic[3],
        lower.tail = FALSE
        
      ),
      pf(
        model[[5]]$fstatistic[1],
        model[[5]]$fstatistic[2],
        model[[5]]$fstatistic[3],
        lower.tail = FALSE
...
herb_models <- map(
  c("Anthocyanins", "Thickness", "Toughness", "LMA", "Shannon"),
  model_summary,
  data = masterant2,
  response = "logit_herbivory"
)
...
patho_models <- map(
  ...
  model_summary,
  data = masterpat2,
  response = "logit_pathogenicity"
)

# E-high
herb_ehigh_models <- map(
  c("Anthocyanins", "Thickness", "Toughness", "LMA", "Shannon"),
  model_summary,
  data = masterant2[masterant2$E_load == "E+", ],
  response = "logit_herbivory"
)

# herb_ehigh_lmlt <- summary(lm(logit_herbivory ~ Thickness, data = masterant2[masterant2$E_load == "E+", ]))
# herb_ehigh_lmlps <- summary(lm(logit_herbivory ~ Toughness, data = masterant2[masterant2$E_load == "E+", ]))
# herb_ehigh_lmaci <- summary(lm(logit_herbivory ~ Anthocyanins, data = masterant2[masterant2$E_load == "E+", ]))
# herb_ehigh_lmlma <- summary(lm(logit_herbivory ~ LMA, data = masterant2[masterant2$E_load == "E+", ]))
# herb_ehigh_lmsdi <- summary(lm(logit_herbivory ~ Shannon, data = masterant2[masterant2$E_load == "E+", ]))

# E-low
herb_elow_models <- map(
  c("Anthocyanins", "Thickness", "Toughness", "LMA", "Shannon"),
  model_summary,
  data = masterant2[masterant2$E_load == "E-", ],
  response = "logit_herbivory"
)
# herb_elow_lmlt <- summary(lm(logit_herbivory ~ Thickness, data = masterant2[masterant2$E_load == "E-", ]))
# herb_elow_lmlps <- summary(lm(logit_herbivory ~ Toughness, data = masterant2[masterant2$E_load == "E-", ]))
# herb_elow_lmaci <- summary(lm(logit_herbivory ~ Anthocyanins, data = masterant2[masterant2$E_load == "E-", ]))
# herb_elow_lmlma <- summary(lm(logit_herbivory ~ LMA, data = masterant2[masterant2$E_load == "E-", ]))
# herb_elow_lmsdi <- summary(lm(logit_herbivory ~ Shannon, data = masterant2[masterant2$E_load == "E-", ]))

# Pathogen E-high
patho_ehigh_models <- map(
  c("Anthocyanins", "Thickness", "Toughness", "LMA", "Shannon"),
  model_summary,
  data = masterpat2[masterpat2$E_load == "E+", ],
  response = "logit_pathogenicity"
)
# patho_ehigh_lmlt <- summary(lm(logit_pathogenicity ~ Thickness, data = masterpat2[masterpat2$E_load == "E+", ]))
# patho_ehigh_lmlps <- summary(lm(logit_pathogenicity ~ Toughness, data = masterpat2[masterpat2$E_load == "E+", ]))
# patho_ehigh_lmaci <- summary(lm(logit_pathogenicity ~ Anthocyanins, data = masterpat2[masterpat2$E_load == "E+", ]))
# patho_ehigh_lmlma <- summary(lm(logit_pathogenicity ~ LMA, data = masterpat2[masterpat2$E_load == "E+", ]))
# patho_ehigh_lmsdi <- summary(lm(logit_pathogenicity ~ Shannon, data = masterpat2[masterpat2$E_load == "E+", ]))

# Pathogen E-low
patho_elow_models <- map(
  c("Anthocyanins", "Thickness", "Toughness", "LMA", "Shannon"),
  model_summary,
  data = masterpat2[masterpat2$E_load == "E-", ],
  response = "logit_pathogenicity"
...
# patho_elow_lmlt <- summary(lm(logit_pathogenicity ~ Thickness, data = masterpat2[masterpat2$E_load == "E-", ]))
# patho_elow_lmlps <- summary(lm(logit_pathogenicity ~ Toughness, data = masterpat2[masterpat2$E_load == "E-", ]))
# patho_elow_lmaci <- summary(lm(logit_pathogenicity ~ Anthocyanins, data = masterpat2[masterpat2$E_load == "E-", ]))
# patho_elow_lmlma <- summary(lm(logit_pathogenicity ~ LMA, data = masterpat2[masterpat2$E_load == "E-", ]))
# patho_elow_lmsdi <- summary(lm(logit_pathogenicity ~ Shannon, data = masterpat2[masterpat2$E_load == "E-", ]))


# Herbivory treatment
regrex_herb2 <- regrex_tables(
  herb_models,
  data_set = "Total",
  "Herbivory"
)
regrex_patho2 <- regrex_tables(
  patho_models,
  data_set = "Total",
  "Pathogen"
)

regrex_herb_ehigh2 <- regrex_tables(
  herb_ehigh_models,
  data_set = "E-high",
  "Herbivory"
)
...
regrex_herb_elow2 <- regrex_tables(
  herb_elow_models,
  data_set = "E-low",
  "Herbivory"
...
regrex_patho_ehigh2 <- regrex_tables(
  patho_ehigh_models,
  data_set = "E-high",
  "Pathogen"
)

regrex_patho_elow2 <- regrex_tables(
  patho_elow_models,
  data_set = "E-low",
  "Pathogen"
)
# regrex_herb <- data.table(
#   LFTs = c(
#     "Anthocyanins (ACI)",
#     "Leaf Thickness (LT) (\U00B5m)",
#     "Leaf Punch Strength (LPS)(N mm^-1)",
...
#   ),
#   data_set = "Total",
#   type_damage = "Herbivory",
#   "Estimate" = c(
#     herb_models[[1]]$coefficients[2],
#     herb_models[[2]]$coefficients[2],
#     herb_models[[3]]$coefficients[2],
#     herb_models[[4]]$coefficients[2],
#     herb_models[[5]]$coefficients[2]
#     # herb_lm_aci$coefficients[2],
#     # herb_lm_lt$coefficients[2],
#     # herb_lm_lps$coefficients[2],
#     # herb_lm_lma$coefficients[2],
#     # herb_lm_sdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     herb_models[[1]]$adj.r.squared,
#     herb_models[[2]]$adj.r.squared,
#     herb_models[[3]]$adj.r.squared,
#     herb_models[[4]]$adj.r.squared,
#     herb_models[[5]]$adj.r.squared
#     
#     # herb_lm_aci$adj.r.squared,
#     # herb_lm_lt$adj.r.squared,
#     # herb_lm_lps$adj.r.squared,
#     # herb_lm_lma$adj.r.squared,
#     # herb_lm_sdi$adj.r.squared
#   ),
#   
#   "F-statistic" = c(
#     herb_models[[1]]$fstatistic[1],
#     herb_models[[2]]$fstatistic[1],
#     herb_models[[3]]$fstatistic[1],
#     herb_models[[4]]$fstatistic[1],
#     herb_models[[5]]$fstatistic[1]
#     
#     # herb_lm_aci$fstatistic[1],
#     # herb_lm_lt$fstatistic[1],
#     # herb_lm_lps$fstatistic[1],
#     # herb_lm_lma$fstatistic[1],
#     # herb_lm_sdi$fstatistic[1]
#   ),
#   
#   "p-value" = c(
#     pf(
#       herb_models[[1]]$fstatistic[1],
#       herb_models[[1]]$fstatistic[2],
#       herb_models[[1]]$fstatistic[3],
#       lower.tail = FALSE
#       # herb_lm_aci$fstatistic[1],
#       # herb_lm_aci$fstatistic[2],
#       # herb_lm_aci$fstatistic[3],
#       # lower.tail = FALSE
#     ),
#     pf(
#       herb_models[[2]]$fstatistic[1],
#       herb_models[[2]]$fstatistic[2],
#       herb_models[[2]]$fstatistic[3],
#       lower.tail = FALSE
#       # herb_lm_lt$fstatistic[1],
#       # herb_lm_lt$fstatistic[2],
#       # herb_lm_lt$fstatistic[3],
#       # lower.tail = FALSE
#     ), 
#     pf(
#       herb_models[[3]]$fstatistic[1],
#       herb_models[[3]]$fstatistic[2],
#       herb_models[[3]]$fstatistic[3],
#       lower.tail = FALSE
#       # herb_lm_lps$fstatistic[1],
#       # herb_lm_lps$fstatistic[2],
#       # herb_lm_lps$fstatistic[3],
#       # lower.tail = FALSE
#     ),
#     pf(
#       herb_models[[4]]$fstatistic[1],
#       herb_models[[4]]$fstatistic[2],
#       herb_models[[4]]$fstatistic[3],
#       lower.tail = FALSE
#       # herb_lm_lma$fstatistic[1],
#       # herb_lm_lma$fstatistic[2],
#       # herb_lm_lma$fstatistic[3],
#       # lower.tail = FALSE
#     ),
#     pf(
#       herb_models[[5]]$fstatistic[1],
#       herb_models[[5]]$fstatistic[2],
#       herb_models[[5]]$fstatistic[3],
#       lower.tail = FALSE
#       # herb_lm_sdi$fstatistic[1],
#       # herb_lm_sdi$fstatistic[2],
#       # herb_lm_sdi$fstatistic[3],
#       # lower.tail = FALSE
#     )
#   )
# )
...
#   ),
#   data_set = "Total",
#   type_damage = c("Pathogen"),
#   "Estimate" = c(
#     patho_lm_aci$coefficients[2],
#     patho_lm_lt$coefficients[2],
#     patho_lm_lps$coefficients[2],
#     patho_lm_lma$coefficients[2],
#     patho_lm_sdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     patho_lm_aci$adj.r.squared,
#     patho_lm_lt$adj.r.squared,
#     patho_lm_lps$adj.r.squared,
#     patho_lm_lma$adj.r.squared,
#     patho_lm_sdi$adj.r.squared
#   ),
#   
#   "F-statistic" = c(
#     patho_lm_aci$fstatistic[1],
#     patho_lm_lt$fstatistic[1],
#     patho_lm_lps$fstatistic[1],
#     patho_lm_lma$fstatistic[1],
#     patho_lm_sdi$fstatistic[1]
#   ),
#   
#   "p-value" = c(
#     pf(
#       patho_lm_aci$fstatistic[1],
#       patho_lm_aci$fstatistic[2],
#       patho_lm_aci$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_lm_lt$fstatistic[1],
#       patho_lm_lt$fstatistic[2],
#       patho_lm_lt$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_lm_lps$fstatistic[1],
#       patho_lm_lps$fstatistic[2],
#       patho_lm_lps$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_lm_lma$fstatistic[1],
#       patho_lm_lma$fstatistic[2],
#       patho_lm_lma$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_lm_sdi$fstatistic[1],
#       patho_lm_sdi$fstatistic[2],
#       patho_lm_sdi$fstatistic[3],
#       lower.tail = FALSE
#     )
#   )
# )
...
#   ),
#   data_set = "E-high",
#   type_damage = c("Herbivory"),
#   "Estimate" = c(
#     herb_ehigh_lmaci$coefficients[2],
#     herb_ehigh_lmlt$coefficients[2],
#     herb_ehigh_lmlps$coefficients[2],
#     herb_ehigh_lmlma$coefficients[2],
#     herb_ehigh_lmsdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     herb_ehigh_lmaci$adj.r.squared,
#     herb_ehigh_lmlt$adj.r.squared,
#     herb_ehigh_lmlps$adj.r.squared,
#     herb_ehigh_lmlma$adj.r.squared,
#     herb_ehigh_lmsdi$adj.r.squared
#   ),
#   
#   "F-statistic" = c(
#     herb_ehigh_lmaci$fstatistic[1],
#     herb_ehigh_lmlt$fstatistic[1],
#     herb_ehigh_lmlps$fstatistic[1],
#     herb_ehigh_lmlma$fstatistic[1],
#     herb_ehigh_lmsdi$fstatistic[1]
#   ),
#   
#   "p-value" = c(
#     pf(
#       herb_ehigh_lmaci$fstatistic[1],
#       herb_ehigh_lmaci$fstatistic[2],
#       herb_ehigh_lmaci$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_ehigh_lmlt$fstatistic[1],
#       herb_ehigh_lmlt$fstatistic[2],
#       herb_ehigh_lmlt$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_ehigh_lmlps$fstatistic[1],
#       herb_ehigh_lmlps$fstatistic[2],
#       herb_ehigh_lmlps$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_ehigh_lmlma$fstatistic[1],
#       herb_ehigh_lmlma$fstatistic[2],
#       herb_ehigh_lmlma$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_ehigh_lmsdi$fstatistic[1],
#       herb_ehigh_lmsdi$fstatistic[2],
#       herb_ehigh_lmsdi$fstatistic[3],
#       lower.tail = FALSE
#     )
#   )
# )  
...
#   type_damage = c("Herbivory"),
#   "Estimate" = c(
#     herb_elow_lmaci$coefficients[2],
#     herb_elow_lmlt$coefficients[2],
#     herb_elow_lmlps$coefficients[2],
#     herb_elow_lmlma$coefficients[2],
#     herb_elow_lmsdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     herb_elow_lmaci$adj.r.squared,
#     herb_elow_lmlt$adj.r.squared,
#     herb_elow_lmlps$adj.r.squared,
#     herb_elow_lmlma$adj.r.squared,
#     herb_elow_lmsdi$adj.r.squared
#   ),
#   "F-statistic" = c(
#     herb_elow_lmaci$fstatistic[1],
#     herb_elow_lmlt$fstatistic[1],
#     herb_elow_lmlps$fstatistic[1],
#     herb_elow_lmlma$fstatistic[1],
#     herb_elow_lmsdi$fstatistic[1]
#   ),
#   
#   "p-value" = c(
#     pf(
#       herb_elow_lmaci$fstatistic[1],
#       herb_elow_lmaci$fstatistic[2],
#       herb_elow_lmaci$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_elow_lmlt$fstatistic[1],
#       herb_elow_lmlt$fstatistic[2],
#       herb_elow_lmlt$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_elow_lmlps$fstatistic[1],
#       herb_elow_lmlps$fstatistic[2],
#       herb_elow_lmlps$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_elow_lmlma$fstatistic[1],
#       herb_elow_lmlma$fstatistic[2],
#       herb_elow_lmlma$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       herb_elow_lmsdi$fstatistic[1],
#       herb_elow_lmsdi$fstatistic[2],
#       herb_elow_lmsdi$fstatistic[3],
#       lower.tail = FALSE
#     )
#   )
#   
...
#   type_damage = c("Pathogen"),
#   "Estimate" = c(
#     patho_ehigh_lmaci$coefficients[2],
#     patho_ehigh_lmlt$coefficients[2],
#     patho_ehigh_lmlps$coefficients[2],
#     patho_ehigh_lmlma$coefficients[2],
#     patho_ehigh_lmsdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     patho_ehigh_lmaci$adj.r.squared,
#     patho_ehigh_lmlt$adj.r.squared,
#     patho_ehigh_lmlps$adj.r.squared,
#     patho_ehigh_lmlma$adj.r.squared,
#     patho_ehigh_lmsdi$adj.r.squared
#   ),
#   
#   "F-statistic" = c(
#     patho_ehigh_lmaci$fstatistic[1],
#     patho_ehigh_lmlt$fstatistic[1],
#     patho_ehigh_lmlps$fstatistic[1],
#     patho_ehigh_lmlma$fstatistic[1],
#     patho_ehigh_lmsdi$fstatistic[1]
#   ),
#   
#   "p-value" = c(
#     pf(
#       patho_ehigh_lmaci$fstatistic[1],
#       patho_ehigh_lmaci$fstatistic[2],
#       patho_ehigh_lmaci$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_ehigh_lmlt$fstatistic[1],
#       patho_ehigh_lmlt$fstatistic[2],
#       patho_ehigh_lmlt$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_ehigh_lmlps$fstatistic[1],
#       patho_ehigh_lmlps$fstatistic[2],
#       patho_ehigh_lmlps$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_ehigh_lmlma$fstatistic[1],
#       patho_ehigh_lmlma$fstatistic[2],
#       patho_ehigh_lmlma$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_ehigh_lmsdi$fstatistic[1],
#       patho_ehigh_lmsdi$fstatistic[3],
#       patho_ehigh_lmsdi$fstatistic[3],
#       lower.tail = FALSE
#     )
#   )
...
#   type_damage = c("Pathogen"),
#   "Estimate" = c(
#     patho_elow_lmaci$coefficients[2],
#     patho_elow_lmlt$coefficients[2],
#     patho_elow_lmlps$coefficients[2],
#     patho_elow_lmlma$coefficients[2],
#     patho_elow_lmsdi$coefficients[2]
#   ),
#   "R^2^~adj~" = c(
#     patho_elow_lmaci$adj.r.squared,
#     patho_elow_lmlt$adj.r.squared,
#     patho_elow_lmlps$adj.r.squared,
#     patho_elow_lmlma$adj.r.squared,
#     patho_elow_lmsdi$adj.r.squared
#   ),
#   "F-statistic" = c(
#     patho_elow_lmaci$fstatistic[1],
#     patho_elow_lmlt$fstatistic[1],
#     patho_elow_lmlps$fstatistic[1],
#     patho_elow_lmlma$fstatistic[1],
#     patho_elow_lmsdi$fstatistic[1]
#   ),
#   "p-value" = c(
#     pf(
#       patho_elow_lmaci$fstatistic[1],
#       patho_elow_lmaci$fstatistic[2],
#       patho_elow_lmaci$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_elow_lmlt$fstatistic[1],
#       patho_elow_lmlt$fstatistic[2],
#       patho_elow_lmlt$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_elow_lmlps$fstatistic[1],
#       patho_elow_lmlps$fstatistic[2],
#       patho_elow_lmlps$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_elow_lmlma$fstatistic[1],
#       patho_elow_lmlma$fstatistic[2],
#       patho_elow_lmlma$fstatistic[3],
#       lower.tail = FALSE
#     ),
#     pf(
#       patho_elow_lmsdi$fstatistic[1],
#       patho_elow_lmsdi$fstatistic[2],
#       patho_elow_lmsdi$fstatistic[3],
#       lower.tail = FALSE
#     )
#   )
# )
...
table4 <- regrex_all |>
  gt(
    groupname_col = c("type_damage", "data_set"),
  ...
  fmt_number(columns = everything(),
             ...
             use_seps = FALSE) |>
  cols_label(
    LFTs = "Leaf Functional Traits",
    estimate = "Estimate",
    rsqrd = md("R^2^~adj~"),
    fstat = md("*F*-statistic"),
    pval = md("*p*-value")
  ) |>
  fmt_stars(columns = c("pval")) |>
  cols_align(align = "right", columns = everything()) |>
  tab_stubhead(label = "Treatment & Data Set") |>
  tab_style(
  ...
  cols_add(dir = ifelse(
      estimate < 0 & pval >= 0.05, "white",
      ifelse(
        estimate < 0 & pval < 0.05, "#dd5129",
        ifelse(
          estimate > 0 & pval < 0.05, "#43b284",
  ...
  cols_label(dir = "") |>
  text_case_match(
    "white" ~ fontawesome::fa("arrow-down"),
    "#dd5129" ~ fontawesome::fa("arrow-down", height = "1.5em", width = "1em"),
    "#43b284" ~ fontawesome::fa("arrow-up", height = "1.5em", width = "1em")
  ) |>
  tab_style(style = cell_text(color = from_column("dir")),
  ...
  tab_footnote(
    footnote = "Positive correlation denoted by upward light green arrow, negative correlation by downward light orange arrow.",
    placement = "auto"
  ) |>
  tab_header(
    title = md("**Summary of simple linear regression models for predicting leaf herbivory and pathogen damage**")
  ) |>
  tab_footnote(
    footnote = "Models are separated by treatment and data set evaluated.",
    location = cells_title()
  )

table4
gtsave(table4, "tables/Aponte_Bolivar_table4_new.html")
```
...
M3REM <- lme(
  logit_herbivory ~ Thickness + LMA + E_load,
...
# summary(M3REM)
# AIC(M3REM) #741.7852 as of 30/OCT/2023

# P3REM
# Best fit model after model selection. See Aim3_leaftraits_Statistical_Analyses.qmd for details.
P3REM <- lme(
  logit_pathogenicity ~ LMA + Thickness,
  weights = varIdent(form = ~ 1 | Species),
  random = ~ 1 |
    Species / E_load / Treatment,
  method = "REML",
  control = ctrl,
  data = masterpat2,
  na.action = na.exclude
...
### Phylogenetic correction ### See Aim3_leaf_traits_statistical_analyses.qmd for more info on model selection. 
# Species list organized by matrix order
species_list <- c("CORDAL",
                  "HEISCO",
                  "THEOCA",
                  "CHRYCA",
                  "DYPTE",
...
phylo_tree <- ape::read.tree("clean_data/phylogeny/iqtree_results/all_species_rbcL_aligned.fasta.treefile")  

# Change tip labels to species acronyms
phylo_tree[["tip.label"]] <- species_list

# Compute the covariance matrix
phylo_vcv <- vcv(phylo_tree, corr = TRUE)  # Compute the covariance matrix

phylo_vcv_clean <- as.matrix(phylo_vcv) %>%
  as.data.frame

# Create a phylogenetic correlation structure (Pagel's “lambda” Correlation Structure)
phylo_cor <- corPagel(1, phy = phylo_tree, form = ~ Species)
# Assuming a λ  of 1 (full phylogenetic effect)

# Models with phylogenetic correction based on Rviewer 1 from Functional Ecology
# Hebivory models
## Fit the model
phylo_herbivory_REML <- lme(
  logit_herbivory ~ Thickness + LMA + E_load,
  weights = varIdent(form = ~ 1 | Species),
  random = ~ 1 | Species,
...
# Pathogen models
## Fit the model
phylo_pathogen_REML <- lme(
  logit_pathogenicity ~ LMA + Thickness,
  # weights = varIdent(form = ~ 1 | Species),
  random = ~ 1 | Species / E_load / Treatment,
  correlation = phylo_cor,
  method = "REML",
  control = ctrl,
  data = masterpat2,
...
## Evaluating model
# AIC(PHY_HREM) # 623.1469
# AIC(PHY_PREM) # 903.0283
...
# Using `huxtable` package to create the same table
...
table5 <- huxreg(
  "Herbivory model" = phylo_herbivory_REML,
  "Pathogen damage model" = phylo_pathogen_REML,
  coefs = c(
    "Intercept" = "(Intercept)",
    "Thickness" = "Thickness",
    "LMA" = "LMA",
  ...
  align = ".",
  ci_level = 0.95,
  tidy_args = list(conf.int = TRUE, effects = "fixed"),
  ...
  insert_row("Random effects", "", "", "", "", after = 7) |>
  set_all_padding(4) |>
  set_outer_padding(0) |>
  set_bold(row = 1, col = everywhere) |>
  # set_bottom_border(row = 1, col = everywhere) |>
...
# In text editor Random effects coefficients will be added for both models using
# summary(phylo_herbivory_REML)
# phylo_herbivory_REML[["coefficients"]][["random"]][["Species"]]
# 
# summary(phylo_pathogen_REML)
# phylo_pathogen_REML[["coefficients"]][["random"]][["Species"]]

# save_as_html(table3, path = "tables/Aponte_Bolivar_table4.html")
# save_as_docx(table2, path = "tables/Aponte_Bolivar_table4.docx")
```
...
upper_lower_damage <- master |>
  mutate(
    herbivory_level = case_when(
      Ant_percent_leafloss >= 70 ~ "high",
      Ant_percent_leafloss %between% list(31, 69) ~ "medium",
      Ant_percent_leafloss < 30 ~ "low"
    ),
    pathogen_level = case_when(
      Pathogen_percent_damage >= 30 ~ "high",
...
supptable_S6 <- gt(
  herb_multipatt_taxonomy |>
    mutate(across(s.high:s.medium, na_if, 0),
      Herbivory =
        case_when(
          s.high == 1 ~ "High",
          s.medium == 1 ~ "Medium",
...
supptable_S7 <- gt(
  patho_multipatt_taxonomy |>
    mutate(across(s.high:s.low, na_if, 0),
      Pathogen =
        case_when(
          s.high == 1 ~ "High",
          s.low == 1 ~ "Low"
...
supptable_S8 <- gt(
  eload_multipatt_taxonomy |>
    mutate(across(`s.E-`:`s.E+`, na_if, 0),
      Treatment =
        case_when(
          `s.E-` == 1 ~ "E-",
          `s.E+` == 1 ~ "E+"
...
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig1.png", plot = suppfig_1, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
```
...
# Comparisons
mycomparisons <- list(
  c("APEIME", "CHRYCA"), c("APEIME", "CORDAL"), c("APEIME", "DYPTE"), c("APEIME", "HEISCO"), c("APEIME", "LACPA"), c("APEIME", "THEOCA"),
...
suppfig_2
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig2.png", plot = suppfig_2, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
```
...
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig3.png", plot = suppfig_3, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
...
suppfig_4
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig4.png", plot = suppfig_4, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
...
suppfig_5
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig5.png", plot = suppfig_5, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
```
...
# Figure S6a-S6b
suppfig_6 <- ggarrange(herb_violin_a, patho_violin_b,
  nrow = 2,
  ncol = 1,
  label.x = -0.015, vjust = 1,
  common.legend = TRUE,
...
suppfig_6
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig6.png", plot = suppfig_6, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
```
...
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig7.png", plot = suppfig_S7, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
```
...
suppfig_S8
# Saving
# ggsave(filename = "./figures/Aponte_Bolivar_suppfig8.png", plot = suppfig_S8, dpi = 300, units = c("mm"), width = 250, height = 200, bg = "white")
