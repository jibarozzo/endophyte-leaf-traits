...
-   GLMs and main effects in tree-style plot.
    ...
    -   Decide to present them as a table.
...
Here I take the loading (PCs) from the **Ant Full PCA** and join them to the ant herbivory (leafloss) data. Thus being able to perform linear regressions from this new dataset (ant_paca + masterant2).
...
The idea here is to see which leaf functional trait and community diversity traits (relative abundance, richness and diversity) impact herbivory and pathogenicity. Only using LMA, thickness and toughness, as these covariates are orthagonal to each other in PCA analysis.
...
The best fit model, considering the eliminatiion of Anthocyanins and Toughness due to high colinearity, is M3REM (AIC 741). It also models fixed effects, not interactions.
