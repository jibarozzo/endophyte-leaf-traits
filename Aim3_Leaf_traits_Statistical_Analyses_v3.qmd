...
-   GLMs and main effects in tree-style plot.
    ...
    -   Decided- to present them as a table.
...
```{r, Post VSEARCH and T-BAS data cleaning, tidy = TRUE}
...
```{r, Total reads boxplot, tidy = TRUE}
...
```{r, Relative abundance boxplot, tidy = TRUE}
# By tree species
...
```{r, RA per E_load bar and violin plot, tidy = TRUE}
# RA per E_load
...
# Saving
#ggsave(filename="./plots/Aim3_Fig1_02JAN2024.png", plot = fig1, dpi=600, units=c("mm"), width = 200, height = 180, bg ="white")
#
...
## Figure S2-S5
```{r, Functional traits violins, tidy = TRUE}
mycomparisons <- list(c("APEIME", "CHRYCA"), c("APEIME", "CORDAL"), c("APEIME", "DYPTE"), c("APEIME", "HEISCO"),c("APEIME", "LACPA"), c("APEIME","THEOCA"),
                      c("CHRYCA","CORDAL"), c("CHRYCA","DYPTE"),c("CHRYCA","HEISCO"), c("CHRYCA","LACPA"), c("CHRYCA", "THEOCA"),
                      c("CORDAL","DYPTE"), c("CORDAL","HEISCO"), c("CORDAL","LACPA"), c("CORDAL","THEOCA"), 
                      c("DYPTE","HEISCO"), c("DYPTE","LACPA"), c("DYPTE","THEOCA"), 
...
#ggsave(filename = "./plots/Aim3_Fig2_02JAN2024.png", plot = grid_pca, dpi=300, units=c("mm"), width = 200, height = 120, bg = "white")
...
```{r, Ant & Pathogen PC1-2, tidy=TRUE}
...
-   Random effect "Species" REDUCES the AIC score. It is important to incorporate in the model!
...
plot(M4REM)
summary(M4REM)
AIC(M4REM) #575.565 # 753 as of 01/NOV/2023
...
From the previous section I can see that model M3 and M5 have the lowest AIC scores. This is achieved by modeling interaction of the explanatory variables. Yet some interactions do not seem to be significant. What is the most parsimonious model I can achieve?
...
The best fit model, considering the elimination of Anthocyanins and Toughness due to high colinearity, is `M3REM` (AIC 741). It also models fixed effects, not interactions.

# Conclusions of model selection using REML model estimation (ANT)

The interaction between `lme(logit_herbivory ~ Thickness  + LMA + E_load, weights = varIdent(form = ~ 1|Species),  random = ~ 1 | Species, method = "REML", control = ctrl, data = masterant2, na.action = na.exclude)` is the best fit model with random effect (Species). We are trying to understand how FEF diversity influences herbivory on an per species/plant basis. The variance structure seems to not be nested. Nesting it increases AIC score. Hence, no need to nest Species/E_load.
...
The final model from this section (P2AIC): `logit_pathogenicity ~ Species + E_load + Thickness`
...
pt <- ggplot(data = masterpat2, aes(y= logit_pathogenicity, x = Toughness, color = E_load)) +
   geom_jitter(aes(shape = Species), size = 1.5, alpha = 0.7) +
  scale_shape_manual(values=c(15:22)) +
  scale_color_manual(values = wes_paleta2)+
  geom_smooth(method=lm, se = F) +
  #stat_regline_equation(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = "~~~~"))) +
...
ptt <- ggplot(data = masterpat2, aes(y= logit_pathogenicity, x= Thickness, color = E_load))+
   geom_jitter(aes(shape = Species), size = 1.5, alpha = 0.7) +
  scale_shape_manual(values=c(15:22)) +
  scale_color_manual(values = wes_paleta2) +
  geom_smooth(method=lm, se=F) +
  ...
  geom_smooth(method = lm, se = T , level = 0.95, na.rm = F, color = "black") +
  stat_regline_equation(label.y = 2.5, label.x = 200, aes(label = ..eq.label..), size = rel(3), color= "black") +
  stat_regline_equation(label.y = 1.5 , label.x = 200, aes(label = ..adj.rr.label..), size = rel(3), color = "black") +
...
plma <- ggplot(data = masterpat2, aes(y= logit_pathogenicity, x= LMA, color = E_load))+
  geom_point(aes(shape = Species), size = 1.5, alpha = 0.7) +
  scale_shape_manual(values=c(15:22)) +
  scale_color_manual(values = wes_paleta2) +
  geom_smooth(method=lm, se=F) +
#  stat_regline_equation(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = "~~~~"))) +
...
# Extracting effect size of model to plot
pdv <- ggplot(data = masterpat2, aes(y= logit_pathogenicity, x= Shannon, color = E_load))+
  geom_jitter(aes(color = E_load, shape = Species), size = 1.5, alpha = 0.7) +
  scale_color_manual(values = wes_paleta2)+
  geom_smooth(method=lm, se=F) +
 # stat_regline_equation(aes(label = paste(..eq.label.., ..adj.rr.label.., sep = "~~~~"))) +
  geom_smooth(method = lm, se = T , level = 0.95, na.rm = F, color = "black") +
  scale_shape_manual(labels= c("A. membranacea", "C. cainito", "C. alliodora", "Dypterix .sp", "H. concinna", "L. panamensis", "T. cacao"),values=c(15:22)) +  #scale_color_manual(values= met.brewer("Cross", n = 8, type = "discrete")) +
  stat_regline_equation(label.y = 2.5, label.x = 1.2, aes(label = ..eq.label..),size = rel(3), color= "black") +
  stat_regline_equation(label.y = 1.5 , label.x = 1.2, aes(label = ..adj.rr.label..), size = rel(3),color = "black") +
  labs(x="Shannon diversity index", y="logit[Pathogen Damage (%)]") +
  theme_minimal(base_size = 10) +
 theme(legend.position="right",
...
#Saving
#ggsave(filename ="./plots/Aim3_Pathogen_regression_01NOV2023.jpg", plot = arranged_with_caption, dpi= 600, units=c("mm"), width=180, height=140, bg = "white")

```

# Table 1
# LMM tables and estimites for ant and pathogen GLMs with `logit_herbivory` and `logit_pathogenicity`
...
```{r, Correcting for Type I errror, tidy = TRUE}
...
multipatt_analysis_sign[ , p.value.bh :=   p.adjust(p.value, method="BH")] #This can also be calculated with `dplyr` and `tidyr`
...
multipatt_analysis_sign[p.value.bh <= 0.05,]
...
## Correlation of Species by leaf damage
...
#Plants with more than 70% leaf damage by ant herbivory per tree Species.
#Highest >70%
upper_herb_damage <- masterant %>%
  group_by(Species) %>%
  filter(Ant_percent_leafloss > 70) %>%
  reframe(across(c(Sample_name, E_load, Ant_percent_leafloss))) 

#Plants with less than 30% leaf damage by ant herbivory per tree Species.
#Lowest < 30%
lower_herb_damage <- masterant %>%
  group_by(Species) %>%
...
```{r, Recoding upper/lower, tidy = TRUE}
# 
...
rownames(cotu) <- cotu$Sample_name #adding sample names as rownames

sample_names <- rownames(cotu) #making vector of sample names

leaf_damage_filter <- inner_join(upper_lower_damage,cotu, by= c("Sample_name", "Species", "E_load"))#Should result in 220 observations and 575 variables

#Removing samples with NA values in all row
  leaf_damage_filter <- leaf_damage_filter %>%
    filter(Sample_name == sample_names)
#Anti join
filter_filter <- anti_join(leaf_damage_filter, cotu, by = c("Sample_name")) #should turn 0 observations and 8 variables

#Lowest leaf damage ant
lower_herb_damage_filter <- semi_join(cotu, lower_herb_damage, by= c("Sample_name")) 
...
```{r, Highest/lowest leaf damage herbivore and pathogen indicator species, tidy = TRUE}
# Recoded dataset for indicator species analysis
#Making vector of groups for analysis
recoded_filtered <- leaf_damage_filter[, 4:ncol(leaf_damage_filter)]

species <- leaf_damage_filter$Species
...
#Making vector of groups for analysis
ltop_pat <- upper_patho_damage_filter[, 4:ncol(upper_patho_damage_filter)]

species9 <- upper_patho_damage_filter$Species

upper_patho_damage_indi = multipatt(ltop_pat, species9, func = "r.g", control = how(nperm=999)) #By using abundance data and not presence-absence we are calculating the "point biserial correlation coefficient"

summary(lpatop_indi, indvalcomp = TRUE)
 

#Lowest leaf damage by pathogen
lower_patho_damage <- lower_patho_damage_filter[,4:ncol(lower_patho_damage_filter)]

species10 <- lower_patho_damage_filter$Species

lbotpat <- multipatt(lower_patho_damage, species10, func = "r.g", control = how(nperm = 999))
...
Paired is not the same as pairwise. Pairwise refers to the comparison each group compared with every other group.
