...
```{r, table1, tidy = TRUE}
# Using `vtable` package
summa_t1 <- st(data2 %>%
  group_by(Species) %>%
  select(c(10:13)) %>%
  rename('Anthocyanins (ACI)' = Anthocyanins, 
           'Leaf Thickness (LT)'=Thickness, 
           'Leaf Punch Strength (LPS)'= Toughness, 
           'Leaf Mass per Area (LMA)' = LMA) %>%
    na.omit(), 
  #out="kable", 
  digits=2, 
  title="Summary statistics for the leaf functional traits of the 7 tropical tree species",
  summ.names = c('N','Mean','Std. Dev.','Min','Max'),
         group = 'Species', group.long = T)

summa_t1

# Using `gtsummary` package
summa2 <- tbl_summary(data2 %>%
...
summa2 <- as_flex_table(summa2)

summa2 <- set_header_labels(summa2, APEIME = "A. membranacea", 
CHRYCA = "C. cainito", 
CORDAL = "C. alliodora", 
DYPTE ="Dypterix .sp", 
HEISCO = "H. concinna", 
...
THEOCA = "T. cacao")
summa2
summa2 <- set_caption(summa2, "Summary statistics for the leaf functional traits of the 7 tropical tree species")

# Using `flextable` package
summa3 <- flextable(data2 %>%
  group_by(Species, E_load) %>%
  select(c(10:13)) %>%
...
summa3
summa3 <- summa3 %>%
  #padding(j = 1, padding.left = 40) %>%
  #padding(j = 3:10, padding.left = 10) %>%
  align(j = 1, align = "right", part = "all") %>%
  align(j = 2:10, align = "center", part = "all") %>%
  italic(j = 1, italic = TRUE, part = "body") %>%
  colformat_double(j = 2:8, big.mark = ",", digits = 3, na_str = "N/A") %>%
  colformat_double(j = 9:10, big.mark = ",", digits = 5, na_str = "N/A") %>%
  set_caption(as_paragraph(as_b("Table 1: Summary statistics for the leaf functional traits of the 7 tropical tree species")), align_with_table = F) %>%
  separate_header() %>%
  autofit()
...
```{r, RA per E_load boxplot, tidy = TRUE}
# RA per E_load
...
LT <- ggplot(data = master, aes(x = Species, y = Thickness), color = Species) +
 ...
 # stat_compare_means(method = "kruskal.test", label = "p.signif") +
  ...
  ylim(100,325)
LT
cp <- compare_means(Thickness ~ Species, data = master, method = "t.test")
...
lps <- ggplot(data = master, aes( x = Species, y = Toughness), group_by(Species)) +
  geom_violin(aes(fill = E_load), outlier.shape = NA) +
  ...
  labs(caption = "*Outliers and singletons removed.") +
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.", label.y = 1) +
  ylim(0.10, 1) 
lps
#ggsave(filename="Aim3_LPS_Species_07062022.png", plot = LT, dpi=600, units=c("mm"), width = 300, height = 300)
...
```{r, Ant & Patthogen PC1-2, tidy=TRUE}
