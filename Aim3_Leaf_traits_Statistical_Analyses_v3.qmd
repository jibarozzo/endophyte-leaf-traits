...
# cor()
corr <- cor(data2[,c(10:13)], y = NULL, use = "complete.obs", method = "pearson")
corr
#Correlation between Anthocyanins and LMA is 0.7
cor.test(data2$Anthocyanins, data2$LMA, method = "pearson")
cor.test(data2$Toughness, data2$LMA, method = "pearson")
...
```{r, Ant Simple Linear Models wit masterant2 data set, tidy = TRUE}
# ANTS:Diversity
# Model with intercept only
...
#M2
M2 <- lme(logit_herbivory ~ Thickness + LMA + E_load, random = ~1 | Species, method = "ML", data = masterant2, na.action = na.exclude)
summary(M2) #Adding the random effect INCREASES AIC score.
AIC(M2)# 756.4149 as of 30/OCT/2023
...
M2.step$anova
AIC(M2.step) #Step:  AIC = 596.744 Ant_percent_leafloss ~ E_load.x  # as of 5/23/23 Final Model:logit_herbivory ~ Thickness + LMA + E_load
...
M3 <- lme(logit_herbivory ~ Thickness + LMA, random = ~1 | Species/E_load, method = "ML", data = masterant2, na.action = na.exclude)

summary(M3) #Nestin E_load in Species term INCREASES AIC score.
AIC(M3) #604.2886 # 1936.182 as of 5/23/23 # 768.2949 06/04/2023
...
M4 <- lme(logit_herbivory ~ Thickness  * LMA  * E_load, random = ~1|Species, method = "ML", data = masterant2, na.action = na.exclude) #Observed richness is confounded by Shannon diversity, probably. The same happens with Toughness and Thickness. 
summary(M4)
AIC(M4) #601.8333 Decreases slightly # 1912.486 as of 5/23/23 # 757.2809  as of 30/OCT/2023 IT INCREASEs
...
plot(M1REM)
summary(M1REM)
AIC(M1REM) # 755.8854 as of 30/OCT/2023 BETTER
...
-   Random effect "Species" REDUCES the AIC score. It is important to incorporate as a randome effect in the model!
...
The best fit model(s), considering the elimination of Anthocyanins and Toughness due to high colinearity, is `M5REM` (AIC 720) and `M3REM` (AIC 741).
