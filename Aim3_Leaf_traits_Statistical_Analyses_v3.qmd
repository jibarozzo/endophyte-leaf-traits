...
#Add adjusted p-value
herb_multipatt_sign[ , p.value.bh :=   p.adjust(p.value, method="BH")] #This can also be calculated with `dplyr` and `tidyr`
...
supptable_S6 <- gt(herb_multipatt_taxonomy |>
  ...
  tab_stubhead(label = "Herbivory level") |>
  tab_stub_indent(
...
#Add adjusted p-value
patho_multipatt_sign[ , p.value.bh :=   p.adjust(p.value, method="BH")] #This can also be calculated with `dplyr` and `tidyr`
