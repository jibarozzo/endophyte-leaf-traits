...
`capscale` and `dbrda` function are very similar [see here](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/rda-and-dbrda/#distance-based%20ReDundancy%20Analysis%20(dbRDA)). 
```{r}
...
## Model 3 and 5 keep species and trial type as the most significant explanatory variables. Yes, the community composition will be different across species but this is due to the inherent leaf traits of each species. Not "species" itself. 
```
...
#ggsave(filename="./plots/Aim3_m3dbRDA_12JAN2024.svg", plot=p_m3dbrda, dpi=600, units=c("mm"), width=300, height=300)
```
...
#### Herbivory damage
```{r, Type I error and data table for herbivory damage, tidy = TRUE}
#Herbivory results
#Extract table of stats
...
#### Pathogen damage
```{r, Type I error and data table for pathogen damage, tidy = TRUE}
