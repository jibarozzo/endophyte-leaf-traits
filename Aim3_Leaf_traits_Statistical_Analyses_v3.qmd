...
-   Pathogen GLMs with `logit_pathogenicity`
...
In June 2023, we have eliminated "Shannon Diversity" as a fixed effects.
...
Model M3REM has the lowest AIC value and the spread of residuals vs fitted values is more constant/even throughout, regardless of optimizer used (AIC = 746.3538).
...
Although this model (M5REM) has the lowest AIC score, it does have predictor variables that are colinear (LMA & Toughness), which confounds the interpretation of the results.
...
t <-ggplot(data = masterant2, aes(y= logit_herbivory, x = Toughness, color = E_load)) +
   geom_jitter(aes(color = E_load, shape = Species), size = 2.2, alpha = 0.7) +
  scale_shape_manual(values=c(15:22)) +
...
-   I opted to remove Anthocyanins and Toughness due to high colinearity.
...
# P3 ####
P3 <- lme(logit_pathogenicity ~ E_load + Thickness + LMA + Shannon, random = ~1 | Species, method = "ML", data = masterpat2, na.action = na.exclude)
