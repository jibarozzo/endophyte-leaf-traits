...
We start here after processing samples with steps outlined in `V-Search_Post_Sequencing.md`
...
# Saving
# saveRDS(csamp, file.path(save.path, "clean_data/clean_sequences.rds"))

# Reading
csamp <- readRDS(file.path(save.path, "clean_data/clean_sequences.rds"))
```
...
# Saving
#saveRDS(tbas, file.path(save.path, "clean_data/tbas_filtered.rds"))
...
data <- read.csv("field_data/Aim3_2019_leaf_traits_datasheet.csv")
...
# Saving
# saveRDS(data, file.path(save.path, "clean_data/leaf_data_raw_analyses.rds"))

# Reading
data <- readRDS(file.path(save.path, "clean_data/leaf_data_raw_analyses.rds"))
...
# Saving
# saveRDS(ndata, file.path(save.path, "clean_data/new_leaf_data.rds"))

# Reading
ndata <- readRDS(file.path(save.path, "clean_data/new_leaf_data.rds"))
...
# Saving
# saveRDS(ndata2, file.path(save.path, "clean_data/new_leaf_data2.rds"))

# Reading
ndata2 <- readRDS(file.path(save.path, "clean_data/new_leaf_data2.rds"))
...
ants <- read.csv("field_data/Aim3_2019_ant_assay_datasheet.csv")
...
# Saving
# saveRDS(ants, file.path(save.path, "clean_data/ants_data.rds"))
...
# patho <- read.csv("field_data/Aim3_2019_patho_assay_datasheet.csv")
# 
# patho <- patho |>
#   separate(Unique.ID, c("Trial_Code", "ELoad", "Species2", "Replicate")) |>
#   unite("Sample_name", Species2:Replicate, sep = "") |>
...
# Saving
# saveRDS(patho, file.path(save.path, "clean_data/pathogen_data.rds"))

# Reading
patho <- readRDS(file.path(save.path, "clean_data/pathogen_data.rds"))
...
# Saving
# saveRDS(OTU, file.path(save.path, "clean_data/otu_table.rds"))

# Taxonomic Table: 1473 OTU by 7 taxonomic ranks
# TAX <- tax_table(tbas)

# Saving
# saveRDS(TAX, file.path(save.path, "clean_data/taxonomy_table.rds"))
...
# Saving
# saveRDS(SAMP, file.path(save.path, "clean_data/samp_data.rds"))
...
# Saving
# saveRDS(pq1, file.path(save.path, "clean_data/phyloseq_main.rds"))
...
# Saving
# saveRDS(pq2, file.path(save.path, "clean_data/phyloseq_pq2.rds"))
...
newpqRA <- transform_sample_counts(newpq1, function(x)
  x / sum(x))
rabun <- subset(otu_table(newpqRA), rownames(otu_table(newpqRA)) %in% notShared)
write.csv(rabun,
          "clean_data/otu_data/Aim3_OTU_Relative_Abundance_singletons_trimmed.csv")
GPfr = filter_taxa(GPr, function(x)
  mean(x) > 1e-5, TRUE)
...
# Saving
#saveRDS(eOTU2, file.path(save.path, "clean_data/eOTU2.rds"))
...
# Saving
# saveRDS(cOTU, file.path(save.path, "clean_data/clean_OTU.rds"))
...
# Saving
# saveRDS(rabun, file.path(save.path, "clean_data/rabun_tidy.rds"))
...
# Saving
# saveRDS(rawra, file.path(save.path, "clean_data/rawra_tidy.rds"))
...
hist(data$Thickness..micro.m., xlim = c(0, 500))
hist(data$Toughness)
hist(data$LMA)
...
# QQnorms ####
qqnorm(rnorm(
  n = length(data$Anthocyanins..aci.),
...
ggsave(
  filename = "./figures/Aim3_FigS4_04JAN2024.png",
  plot = suppfig_4,
  dpi = 600,
  units = c("mm"),
  width = 250,
...
suppfig6a_compare_means <- compare_means(
  Ant_percent_leafloss ~ E_load,
  data = master,
  #group.by = "Species",
  method = "t.test",
...
#Roughly the same outcome as pairwise_t_test.
suppfig6b_compare_means <- compare_means(
  Ant_percent_leafloss ~ E_load,
  data = master,
  group.by = "Species",
  method = "t.test",
...
t.test(
  x$Ant_percent_leafloss,
  y$Ant_percent_leafloss,
  method = "t.test",
  p.adjust.method = "BH",
  paired = TRUE,
  alternative = "two.sided",
...
#ggpubr package used for t.tests and plotting t-test comparisons
#E+ vs E- for all specie. To compare means of pathogen damage (%) between E+ and E- samples per species un-comment `group.by = "Species"` and change method to  "anova" in compare_means.
suppfig7a_compare_means <- compare_means(
...
#Roughly the same outcome as pairwise_t_test.
suppfig7b_compare_means <- compare_means(
  Pathogen_percent_damage ~ E_load,
  data = master,
  group.by = "Species",
  method = "anova",
...
# Fig. S7a
patho_violin_a <- ggplot(data = master,
                         aes(x = E_load, y = Pathogen_percent_damage, fill = Treatment)) +
  geom_violin() +
  geom_point(
    aes(color = E_load),
    position = position_jitterdodge(dodge.width = 0, jitter.width = 0.2),
    alpha = 0.5,
    size = 2
  ) +
  geom_crossbar(
  ...
  scale_fill_manual(name = "Pathogen treatment ",
...
auto <- autoplot((pca),
                 data = data.frame(ndata),
                 # For some reason this functions require data to be data= data.frame()
                 alpha = 0,
                 #Setting alpha to zero render the automatic circle point null.Manipulate shapes with geom_point().
                 loadings = TRUE,
...
ant_pc1 <- ggplot(ant_pcalm, aes(PC1, Ant_percent_leafloss)) +
  ...
  stat_regline_equation(
    ...
    size = 7,
    aes(label = paste(
      ..eq.label.., sprintf("italic('p')~'%s'", ant_p1), sep = "~~~~"
    ))
  ) +
  scale_color_manual(
    labels = c(
...
pat_pc1 <- ggplot(pat_pcalm, aes(PC1, Pathogen_percent_damage)) +
  ...
  stat_regline_equation(
    ...
    size = 7,
    aes(label = paste(
      ..eq.label.., sprintf("italic('p')~'%s'", pathogen_p1), sep = "~~~~"
    ))
  ) +
  scale_color_manual(
    labels = c(
...
pat_pc2 <- ggplot(pat_pcalm, aes(PC2, Pathogen_percent_damage)) +
  ...
  stat_regline_equation(
    ...
    size = 7,
    aes(label = paste(
      ..eq.label.., sprintf("italic('p')~'%s'", pathogen_p2), sep = "~~~~"
    ))
  ) +
  scale_color_manual(
    labels = c(
...
# Brough E_load back into the fold given Functional Ecology's reviewer comments
#Terms: otu_e.dist ~ Thickness + Toughness + Anthocyanins + LMA
m3dbRDA <- dbrda(otu_e.dist ~ Anthocyanins + Toughness + Thickness + LMA + E_load, data= ndata2, dist="bray", na.action = na.omit)
anova_m3dbRDA <- anova.cca(m3dbRDA, by="margin")
saveRDS(anova_m3dbRDA, file = "./clean_data/anova_m3dbRDA.rds")
...
No interactions yet, checking random effects. \*\*Notice the use of `Treatment` as one of the fixed terms. This is a level that is not present in the Ant dataset. I am including LMA and Treatment as fixed terms to see if they are significant.
...
I believe the best way to represent the model coefficients is through a table, not an estimate plot.

# GLMMs with correction for host species phylogeny

Reviewers from Functional Ecology Journal requested that we include a correction for host species phylogeny given that some of the host species are more related than other (are within the same family). This suggestion was given on 07-15-2024.

The approach is the following:

1.  Find a recent angiosperm phylogenetic tree.

    -   From the [Royal Botanic Gardens Kew Tree of Life](https://treeoflife.kew.org/tree-of-life)

    -   [General Information](https://sftp.kew.org/pub/treeoflife/).

    -   Release information for [treeoflife.all_support_values.3.0.tree](https://sftp.kew.org/pub/paftol/current_release/tree/species/treeoflife.all_support_values.3.0.tree) 2023-05-02 10:10 2.9M.
    - @kress2009: https://pnas.org/doi/full/10.1073/pnas.0909820106
    - @janssens2020: https://bdj.pensoft.net/article/39677/

2.  Corroborate that it includes the species we need:

In Kew Tree of Life

        
    -   *Theobroma cacao* (Malvaceae) -> Yes
    -   *Dipteryx* sp. (Fabaceae) -> Yes (Dipteryx odorata is present)
    -   *Lacmellea panamensis* (Apocynaceae) -> Yes
    -   *Apeiba membranacea* (Malvaceae) -> No (Apeiba schomburgkii is present)
    -   *Heisteria concinna* (Olacaceae) -> No (Heisteria parvifolia is present)
    -   *Chrysophyllum cainito* (Sapotaceae) -> Yes
    -   *Cordia alliodora* (Cordiaceae) -> No (Cordia myxa is present which is the type species. Is is also under Boraginaceae)


In @kress2009

| **Species**                   | **Family**        | **Gene:**                         | **Notes**           |
|-------------------------------|-------------------|-----------------------------------|---------------------|
| *Theobroma cacao*             | Malvaceae         | rbcL, matK, psbA                  |                     |
| *Dipteryx* sp.                | Fabaceae          | rbcL, matK, psbA                  | *Dipteryx oleifera* |
| *Lacmellea panamensis*        | Apocynaceae       | rbcL, matK, psbA                  |                     |
| *Apeiba membranacea*          | Malvaceae         | rbcL                              |                     |
| *Heisteria concinna*          | Olacaceae         | rbcL, matK, psbA                  |                     |
| *Chrysophyllum cainito*       | Sapotaceae        | rbcL, matK, psbA                  |                     |
| *Cordia alliodora*            | Cordiaceae        | rbcL, matK, psbA                  |                     |
...
In @janssens2020
    ...
    -   *Theobroma cacao* (Malvaceae) -> Yes
    -   *Dipteryx* sp. (Fabaceae) -> Yes (Dipteryx odorata is present and others are included)
    -   *Lacmellea panamensis* (Apocynaceae) -> Yes
    -   *Apeiba membranacea* (Malvaceae) -> Yes
    -   *Heisteria concinna* (Olacaceae) -> Yes
    -   *Chrysophyllum cainito* (Sapotaceae) -> Yes (Other spp. are included)
    -   *Cordia alliodora* (Cordiaceae) -> No (Cordia myxa and other Cordia spp. are included)


3.  Compute covariance matrix of tree.

4.  Include covariance matrix in the model.

I opted to build my own phylogenetic tree by downloading reference genomes from @kress2009 and using `fastreeR` package to build tree and calculate the covariance matrix. I will use the *rbcL* gene to build phylogeny since it is highly conserved (and slowe evvolving) and it is available for all species. I be following this [vignette](https://bioconductor.riken.jp/packages/3.15/bioc/vignettes/fastreeR/inst/doc/fastreeR_vignette.html) as a reference.

**FASTA files from @kress2009**

From NCBI:

-   Theobroma cacao:

    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981898.1

-   Dipteryx sp. (loeifera): 
    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981725.1
    
- Chrysophyllum cainito:
    - *rbcL*:https://www.ncbi.nlm.nih.gov/nuccore/GQ981702.1

- Cordia alliodora:
    - https://www.ncbi.nlm.nih.gov/nuccore/GQ982196
    - https://www.ncbi.nlm.nih.gov/nuccore/GQ981972
    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981712

- Lacmellea panamensis: 
    - GQ982266
    - GQ982028
    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981782.1

- Apeiba membranacea:
    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981666.1?report=fasta

- Heisteria concinna:
    - *rbcL*: https://www.ncbi.nlm.nih.gov/nuccore/GQ981761.1
    

#### MUSCLE (v.5.1.0-1) alignment of slow evolving coding gene *rbcL*

```
muscle -align all_species_rbcL.fasta -output all_species_rbcL_aligned.fasta
```

#### IQ-TREE (v.2.3.6-1)

Using IQ-TREE to build a phylogenetic tree through the web browser. I ran a couple of tests to understand how it works. Then I proceed to run commands on CLI locally. 


**set seed 317582
```
iqtree -s all_species_rbcL_aligned.fasta -m TESTNEW -bb 1000 -alrt 1000 -seed 317582 --undo -safe
```
Parameters `--undo` was used to repeat analysis and `-safe` to avoid outpust warnings.

#### FastreeR: Calculating distance and arranging DF
```{r}
# Calculating distances from fasta file
myFastaDist <- fastreeR::fasta2dist(
  "/home/baponte/Boxx/Dissertation/Leaf_Traits_Panama/Data/Aim3_Sequence_analyses/clean_data/phylogeny/all_species_rbcL_aligned.fasta",
  kmer = 6
)
...
plot(myFastaTreeStats, ann = FALSE, cex = 01)



# Rename column and row names of distance matrix
# Pulled species list acronyms
# species_list <- master %>%
#   select(Species) %>%
#   unique() %>%
#   pull()
...
myFastaDist_clean <- as.matrix(myFastaDist)

# New row names
rownames(myFastaDist_clean) <- species_list

# New column names
colnames(myFastaDist_clean) <- species_list
```

Using `fastreeR::fasta2dist` function calculates "d2_S type dissimilarity measurement between the n sequences (which can represent samples) of a FASTA file. See doi:10.1186/s12859-016-1186-3 for more details." Upon further scrutiny, the algorithm employed by this function is adequate for unaligned sequences. From the authors:
```
We describe a scalable multi-tiered binning algorithm that combines frequency and compositional features to cluster unassembled reads, and demonstrate i) significant runtime performance gains of the developed modules against state of the art software, obtained through parallelization and the efficient use of large lock-free concurrent hash maps, ii) its relevance for clustering unassembled reads from high complexity (e.g., harboring 700 distinct genomes) samples, iii) its relevance to experimental setups involving multiple samples, through a use case consisting in the “de novo” identification of sequences from a target genome (e.g., a pathogenic strain) segregating at low levels in a cohort of 50 complex microbiomes (harboring 100 distinct genomes each), in the background of closely related strains and the absence of reference genome
```

I have aligned my sequences so it is probably not the best application. I  think `ape::vcv` function is more adequate for caculation covariance matrix and incorporating in GLMMs. 


## GLMMs with phylogenetic correction

```{r, Phylogenetic correlation}
...
Asking ChatGPT what are the differences between `ape:vcv` and `ape:corPagel` and their different use cases: 
| **Aspect**              | **`ape::vcv`**                                                   | **`ape::corPagel`**                                               |
|-------------------------|------------------------------------------------------------------|-------------------------------------------------------------------|
| **Purpose**             | Computes the variance-covariance matrix of a phylogenetic tree    | Creates a phylogenetic correlation structure using Pagel's lambda |
| **Output**              | Variance-covariance matrix                                        | Correlation matrix or structure usable in mixed models            |
| **Phylogenetic Signal** | Assumes full phylogenetic signal (based on tree topology)         | Allows varying levels of phylogenetic signal (0 ≤ λ ≤ 1)          |
| **Typical Use Case**    | Generalized Least Squares (GLS), phylogenetic independent contrasts | Phylogenetic GLS (PGLS), mixed-effects models                     |
| **Flexibility**         | Less flexible, assumes tree fully explains covariance             | More flexible, adjusts for different degrees of phylogenetic dependence |


This gives me a good idea of how I can use them for GLMMs analyses. 


```{r, Phylogenetic GLMM}
# Starting with model M3REM: the best fit model before including phylogeny
#Optimizers
ctrl <- lmeControl(opt='optim')
...
## Evaluating models
AIC(phylo_herbivory_REML)
AIC(M3REM)

plot_model(phylo_herbivory_REML, type = "diag")
plot_model(M3REM, type = "diag")
plot_models(phylo_herbivory_REML, M3REM)
...
plot_model(phylo_pathogen_REML, type = "diag")
plot_model(P3REM, type = "diag")
plot_models(phylo_pathogen_REML, P3REM)
```

This looks good, but when I went to make a table with `huxtable::huxreg` I received the error: *Error in intervals.lme(PHY_HREM) : cannot get confidence intervals on var-cov components: Non-positive definite approximate variance-covariance. Consider 'which = "fixed"'*. Both models return this erros. Searching on the web revealed that this is probably due to an overfitted model or that there is not enough data for the model structure. According to ChatGPT: *This error usually indicates that the model is having difficulty estimating the variance-covariance components for the random effects, which could be due to several reasons, such as model misspecification, insufficient data, or issues with the random effects structure.*. There are also various Stack Overflow threads discussing the matter, see [here](https://stats.stackexchange.com/questions/444995/scale-of-time-covariate-in-corcar1-matters) and [here](https://stackoverflow.com/questions/53409315/extracting-confidence-intervals-from-lme-model). 

Basically adding the phylogenetic covariance structure over complicated things because I am also modelling variance structure by host species. Both ar ways for accounting for variance in the model.


```{r, Phylogenetic GLMM}
# Models are to complex for the data...

# Controls with 1000 iterations
iteration_ctrl <- lmeControl(opt = "optim", msMaxIter = 1000)
...
# Extracting fixed term Confidence Intervals
phylo_herb_fixed_termsCI <- intervals(phylo_herbivory_REML, which = "fixed")
...
aov(M3REM)
phylo_patho_fixed_termCIs <- intervals(phylo_pathogen_REML, which = "fixed")
broom::tidy

# Tidying model confidence intervals
tidy(phylo_pathogen_REML, effects = "fixed", conf.level = 0.95, conf.int = TRUE)
# tidy() from the broom/generics package helps tidy model coefficients and CIs. This function is used by huxtable::huxreg. Good to know for making the table for models.

# Saving models
save(phylo_herbivory_REML, phylo_pathogen_REML, file = "./clean_data/glmm_phylo_correct_models.rda")
```
...
For herbivory damaged leaves: Indicator/correlation species analysis for samples with the highest leaf damage (\>70%) we see 13 OTUs siginificantly assciated, while only 1 OTU for the lowest leaf damage (\<30%). For samples in the medium range we see that 3 OTUs are significantly correlated with the group.

For pathogen damaged leaves: Zero OTUs significantly correlated with the leaves that were damaged \<30% by pathogen. On the other hand, 13 OTUs are significantly correlated with plants that experienced \>30% leaf damage.
