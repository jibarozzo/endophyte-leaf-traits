...
Finding a recent angiosperm phylogenetic tree that included all the species in this study is hard. I will opt to build my own phylogenetic tree by downloading reference genomes and using `fastreeR` package to build tree and calculate the covariance matrix. I be following this [vignette](https://bioconductor.riken.jp/packages/3.15/bioc/vignettes/fastreeR/inst/doc/fastreeR_vignette.html) as a reference.
...
#### MUSCLE alignment of slow evolving coding gene *rbcL*
...
```{r}
library(fastreeR)
tree_test <- read.tree(file = "clean_data/phylogeny/genome_datasets/all_species_rbcL_aligned.fasta.treefile")

myFastaDist <- fastreeR::fasta2dist("/home/baponte/Boxx/Dissertation/Leaf_Traits_Panama/Data/Aim3_Sequence_analyses/clean_data/phylogeny/genome_datasets/all_species_rbcL_aligned.fasta", kmer = 6)

myFastaTree <- fastreeR::dist2tree(inputDist = myFastaDist)
plot(ape::read.tree(text = myFastaTree), direction = "right", cex = 1)
ape::add.scale.bar()
