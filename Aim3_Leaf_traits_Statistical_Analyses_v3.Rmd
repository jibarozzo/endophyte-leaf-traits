...
setwd("C:/Users/boloq/Box/Tulane/Bioinformatics/Aim3_Sequence_analyses")
#Note that H drive has shortcuts to Google Drive, hence the long file path.
save.path <- "C:/Users/boloq/Box/Tulane/Bioinformatics/Aim3_Sequence_analyses"
```
...
vif(lm( ~ Anthocyanins + Thickness + Toughness + LMA, data = ndata))
...
=======
  labs(title = "Ant only PCA", caption = expression("ACI = anthocyanins, LPS = leaf punch strength, LMA = leaf mass per area, LT = leaf thickness. All leaf replicates per species ("~ italic("n") ~ "= 210).")) +
  theme(plot.title = element_text(color="red", size = 14, face="bold.italic"))
  
>>>>>>> 6f4fc8932ee2e440eaec76e4a6a7a37034880845
...
Saving as grid plot
```{r}
antpca_arranged <- ggarrange(ant.pca, ant_pc1, ant_pc2, nrow = 2, ncol=2, common.legend = TRUE, legend="right")
...
p <- cor.test(ant_pcalm$Ant_percent_leafloss, ant_pcalm$PC2)$p.value

p=format.p(p) # This is just to make the p-value nicer. 
...
<<<<<<< HEAD
  ...
  theme(plot.title = element_text(color="red", size = 14, face="bold.italic")) +
 annotate(geom="text",label=sprintf("italic('p')~'%s'",p),parse=TRUE,x=-1,y=65
          ) +
    guides(color = "none")  +      #To turn off color legend
  guides(fill = "none") + #To turn off fill legend
  guides(shape = "none")
...
#ggsave(filename = "./Plots/Aim3_AntPCS_07102023.png", plot = ant_pca_with_caption, dpi=300, units=c("mm"), width = 200, height = 180, bg = "white")
```
