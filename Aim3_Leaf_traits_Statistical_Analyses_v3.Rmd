...
#setwd("C:/Users/boloq/Desktop/Sequence_analysesLAPTOp")
#Note that H drive has shortcuts to Google Drive, hence the long file path.
```
...
#data
traitpath <- "H:/.shortcut-targets-by-id/0B9v0CdUUCqU5VVR4a3BvNHM1Z28/VBL_users/Grad_Students/Bolivar/Dissertation/Leaf_Traits_Panama/Data/Sequence_analyses"
data <- read.csv(file.path(traitpath, "Aim3_Bolivar_Summer_2019_Leaf_Traits_datasheet.csv"))
data2 <- data %>% #data set created on May 23, 2023 for analysis with raw data.
...
#ndata
ndata <- data %>% #This data set can also be used to conduct analysis with "raw" data . It is summarized by leaf. 
...
The taxonomic assignment is done with the `phyloseq` package. For reference <https://joey711.github.io/phyloseq/>.
...
ntaxa(pq2) #1219 taxa and 78 samples
...
auto <- autoplot((pca), data = data.frame(ndata), # For some reason this functions require data to be data= data.frame()
  ...
  labs(caption = expression("ACI = anthocyanins, LPS = leaf punch strength, LMA = leaf mass per area, LT = leaf thickness. All leaf replicates per species ("~ italic("n") ~ "= 467)."))
...
p1 <- plot_ordination(newpq1, r, type = "samples", color = "Trial_Code", title = "Community Composition by Trial Code") +
 ...
 labs(caption = "Trials for ant and pathogen data sets go from T1-T5. Repeated surveys for ant data set are > T6") +
  facet_wrap(~Species)
p1 #Making these plots with Mareli. The main purpose is to understand community similarity accros trials and time points. -June 2023
...
# # Samples 
# p2 <- plot_ordination(newpq1, r, type="samples", color="Species")
...
After meddling with this for a little bit, I do not like how this plots my data. I find it confusing to edit and know what it is plotting exactly. I will plot these ordinations by suing the data frames cleaned and trimmed with phyloseq. -BAR 06/19/2022
Get back to this when linear models are finished and dbRDA models too.
