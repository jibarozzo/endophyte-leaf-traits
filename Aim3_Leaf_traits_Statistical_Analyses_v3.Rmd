...
>>>>>>> a39298f3af6931bd5d2dfe72231ce7296add48f8
library("mgcv")
library("phyloseq")
library("ape")
library("metagMisc")
...
As of 06/04/2023: The final model as a results of `setAIC()` is: logit_herbivory \~ LMA + Species + Thickness + E_load
...
Model M3REM has the lowest AIC value and the spread of residuals vs fitted values is more constant/even throughout.
...
The interaction between \logit?h
erbivory \~ Anthocyanins : Toughness : Thickness : LMA :E_load, random = \~1 \| Species, weights = varIdent(form = \~ 1\|Species) is the best fit model with random effect (Species).
