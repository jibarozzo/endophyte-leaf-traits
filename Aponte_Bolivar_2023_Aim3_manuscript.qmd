...
Additionally, to facilitate comparison between group levels we used the `pairwise_t_test` function from the `rstatix`  package [@kassambra2023a] when we compared pathogen (%) damage means. This function also wraps and extends base *R* functions. We adjusted *p* values to account for false discovery rates in multiple comparisons by using "BH" method [@benjamini1995].
