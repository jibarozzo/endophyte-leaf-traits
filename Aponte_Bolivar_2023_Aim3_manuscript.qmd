...
The dbRDA analyses revealed that  6.34% of the variance is accounted for by the leaf functional traits, the constraining variables. 



We observed general differences in leaf functional traits among species (Table 2). For most leaf traits we did not find significant differences between treatments (E- and E+) when we combined all host species (Fig S2a, - S5a) but at the individual species level there were differences (Fig. S2b – S5b). 
**IS There something we can say here that relates directly back to the hypothesis? Like, what are the short lived leaves. Did their traits differ? What are the long lived leaves, did their traits differ with endophytes?**
...
#GRAPHING dbRDA  with color by continous leaf lifespan
#plot using ggplot2
names(m3dbRDA)
B <- summary(m3dbRDA)
